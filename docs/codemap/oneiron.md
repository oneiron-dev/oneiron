...
| `src/batch/gate_staging.rs` | src | s | 3 crate-vis | — | — |
...
| `src/batch/put_apply/put_staging.rs` | src | s | 9 crate-vis | — | Body/index/edge row staging helpers shared by the put and update paths |
...
| `src/batch/txn_builder.rs` | src | m | 1 struct · 15 fn · 11 crate-vis | TxnBatchBuilder | — |
...
| `src/gate/doors/staged_claim_gate.rs` | src | s | 8 crate-vis | — | Materialize the preflight decision from the same write transaction, exactly once |
...
| `src/gate/mod.rs` | src | s | 3 re-export · 21 crate-vis | — | DEC-0005 Gate policy manifest resolver |
...
| `src/llm/mod.rs` | src | s | 1 mod · 11 re-export · 4 crate-vis | — | Engine-facing LLM invocation seam |
...
| `src/note.rs` | src | s | 2 struct · 2 enum · 2 fn · 1 const · 3 re-export · 6 crate-vis | NoteBody, NoteScope, NoteWriteEnvelope, TakeTarget | Governed NOTE cores, PACK kind descriptors, and per-note editable documents |
...
| `src/note/kinds.rs` | src | s | 2 struct · 3 enum · 6 fn · 3 crate-vis | ContextDefault, ExtractionDefault, NoteKind, NoteKindDescriptor, RetentionDefault | Vault-resident PACK descriptor records; the shipped kinds are data, not variants |
...
| `src/sync/mod.rs` | src | s | 16 mod · 14 re-export · 2 crate-vis | — | CRDT sync layer for Oneiron |
