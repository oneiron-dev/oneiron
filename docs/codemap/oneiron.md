...
| `src/claim/scoped_read.rs` | src | m | 1 struct · 14 fn · 2 re-export · 5 crate-vis | ScopedRead | The policy-gated read lane: [`ScopedReadActorKey`], [`ScopedRead`], and the admission/filtering surface that… |
...
| `src/ingest/admission.rs` | src | m | 2 struct · 7 fn | ImportedEvidenceAdmission, ImportedEvidenceEntityResolution | Imported-evidence admission: typed claim and entity intake with JSON-to-MessagePack bridging |
...
| `src/llm/mod.rs` | src | s | 7 mod · 17 re-export · 5 crate-vis | — | Engine-facing LLM invocation seam |
...
| `src/store/mod.rs` | src | s | 1 mod · 11 re-export · 9 crate-vis | — | LMDB store: one environment per vault plus the 28 named databases pinned by the ARCH-0019 manifest, and the… |
...
| `src/sync/mod.rs` | src | s | 18 mod · 14 re-export · 2 crate-vis | — | CRDT sync layer for Oneiron |
