...
| `src/batch/txn_builder.rs` | src | m | 1 struct · 14 fn · 9 crate-vis | TxnBatchBuilder | — |
...
| `src/claim/scoped_read.rs` | src | m | 2 struct · 22 fn · 3 crate-vis | ScopedRead, ScopedReadActorKey | The policy-gated read lane: [`ScopedReadActorKey`], [`ScopedRead`], and the admission/filtering surface that… |
...
| `src/lib.rs` | src | m | 161 mod · 71 re-export · 26 crate-vis | — | Long-context memory engine: an LMDB vault, a write Gate, retrieval pipelines and host surfaces |
...
| `src/store/structural_kind_registry/rekey.rs` | src | m | 4 crate-vis | — | One-shot byte-space v3.1 type-byte migration over entities, type_index, counters, and registry rows |
...
| `src/sync/transport.rs` | src | m | 2 struct · 1 enum · 20 fn · 27 const · 1 mod | EncodedFrame, EphemeralWireState, TransportError | Wire protocol encoding/decoding for WebSocket transport |
...
| `src/vault/actors_memory.rs` | src | m | 1 struct · 21 fn · 1 crate-vis | ActorBound | Vault actor binding, structural kinds and code-memory attachment |
