...
|---|---|---|---|---|
| [oneiron](codemap/oneiron.md) | Long-context memory engine: an LMDB vault, a write Gate, retrieval pipelines and host surfaces | 1583 | 558 | 1 |
...
| [oneiron-server](codemap/oneiron-server.md) | Oneiron CRDT sync server library | 194 | 75 | 0 |
...
| `authority` | dir | 46 | m | yes | AUTHORITY_LOG record substrate |
...
| `claim` | dir | 29 | m | yes | CLAIM body ABI + typed Claim API (ARCH-0003, pinned decisions D11/D17/D18) |
...
| `code_run` | dir | 31 | m | yes | Host-side skeleton for first-party `self.*` code-mode calls |
...
| `connector_key` | dir | 19 | m | yes | Connector-key registry records with effector budgets for OF-277 GOV-01 |
...
| `context_board` | dir | 35 | m | — | Typed Context Board render projections |
...
| `credential_door` | dir | 11 | m | — | ARCH-0068 RC4 — the credential door (CSTDY-02) |
...
| `dreamer_consolidation` | dir | 30 | m | yes | Dreamer consolidation algorithm + reflection gap scan (ONE-1289, DREAM-002; DESIGN-PIN-20260710 Part A) |
...
| `federation` | dir | 14 | m | yes | Federation grant record substrate |
| `feedback` | dir | 6 | m | — | Engine feedback channel: bundle wire contract, consent, dispatch, export |
| `fusion` | file+dir | 2 | s | — | — |
| `gate` | dir | 72 | m | yes | DEC-0005 Gate policy manifest resolver |
| `genui` | dir | 9 | m | — | OF-336 generated-UI component contract |
| `git_wire` | dir | 21 | m | — | Engine-owned typed git subprocess boundary (ONE-1903, RC6/ARCH-0068) |
...
| `ingest` | dir | 17 | m | yes | Ingest source registry and source-local normalization |
...
| `llm` | dir | 35 | m | — | Engine-facing LLM invocation seam |
...
| `origin` | dir | 34 | m | yes | Vault-as-origin serving plane (ARCH-0068 Phase A) |
...
| `skill_hub` | dir | 11 | m | yes | Skill-hub records, provenance aliases, adapter contracts, and update gates |
...
| `store` | dir | 46 | m | — | LMDB store: one environment per vault plus the 28 named databases pinned by the ARCH-0019 manifest, and the… |
...
| `sync` | dir | 81 | m | yes | CRDT sync layer for Oneiron |
...
| `api` | dir | 131 | m | HTTP query routes for web dashboard access |
...
| `handler` | dir | 12 | m | WebSocket upgrade handler and connection lifecycle |
...
| `runtime` | dir | 6 | m | — |
| `server` | dir | 8 | m | Sync server state and maintenance jobs, split by concern |
