...
| D02 (G2) | Second corpus / unchanged-engine number absent | **Pending:** both exact manifests, unchanged-engine lanes, guarded-native-v3 lanes and LibreOffice lanes are complete and pinned. Existing-process activation restored SpreadsheetBench Excel ownership after row 2,128; the unchanged oracle resumed from 2,077 reverified completed outputs. FUSE fresh Excel and same-truth comparisons remain unrun. Saved-cache results cannot close this finding or enable native defaults. |
