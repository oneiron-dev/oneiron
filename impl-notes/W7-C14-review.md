...
| D02 (G2) | Second corpus / unchanged-engine number absent | **Pending / Office blocked:** both exact manifests, unchanged-engine lanes, guarded-native-v3 lanes and LibreOffice lanes are complete and pinned. SpreadsheetBench Excel stopped at row 2,128; bounded ownership reads no longer return an identity. Lock/staging are preserved until owner-side recovery. FUSE fresh Excel and same-truth comparisons remain unrun. Saved-cache results cannot close this finding or enable native defaults. |
