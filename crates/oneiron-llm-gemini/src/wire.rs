...
pub(super) fn build_request(
    ...
    let calls: std::collections::BTreeMap<_, _> = request
        ...
        .filter_map(|part| match part {
            ContentPart::ToolCall { call_id, name, .. } => Some((call_id.as_str(), name.as_str())),
            _ => None,
        })
        .collect();
    ...
    for message in &request.messages {
        let parts = message
            .content
            .iter()
            .map(|part| encode_part(part, &calls))
            .collect::<LlmResult<Vec<_>>>()?;
        if message.role == LlmMessageRole::System {
            system.extend(parts);
        } else {
            contents.push(json!({"role":if message.role==LlmMessageRole::Assistant {"model"}else{"user"},"parts":parts}));
        }
    }
    let mut generation = serde_json::Map::new();
    for (name, value) in &request.params {
        let wire_name = match name.as_str() {
    ...
    if let Some(options) = request.provider_options.get("gemini") {
        ...
        for (name, value) in options {
            if !matches!(
                name.as_str(),
                "safetySettings" | "toolConfig" | "cachedContent"
            ) {
...
fn encode_part(
    part: &ContentPart,
    calls: &std::collections::BTreeMap<&str, &str>,
) -> LlmResult<Value> {
    Ok(match part {
        ...
        ContentPart::ToolCall {
            ...
            name,
            input,
        } => json!({"functionCall":{"id":call_id,"name":name,"args":input}}),
        ContentPart::ToolResult {
            call_id,
            output,
            is_error,
        } => {
            json!({"functionResponse":{"id":call_id,"name":calls.get(call_id.as_str()).ok_or(FatalLlmError::InvalidRequest)?,"response":{"output":output,"is_error":is_error}}})
        }
        ContentPart::Image {
            media_type,
            image: ImageContent::Base64 { data },
