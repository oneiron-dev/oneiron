...
impl<T: GeminiTransport> GeminiBackend<T> {
    pub fn from_registry(vault: &oneiron::Vault, transport: T) -> oneiron::Result<Self> {
        Ok(Self {
            transport,
            models: vault
                .model_catalog_entries(oneiron::llm::registry::ModelWireFormat::Gemini)?
                .into_iter()
...
impl<T: GeminiTransport> LlmBackend for GeminiBackend<T> {
    ...
    fn generate<'a>(
        ...
        Box::pin(async move {
            ...
            let response = self
                .transport
                .execute(build_request(entry, &request, false)?, lease)
                .await?;
    ...
    fn stream<'a>(&'a self, request: LlmRequest, lease: &'a BudgetLease) -> LlmStreamResult<'a> {
        ...
        let source = self
            .transport
            .stream(build_request(entry, &request, true)?, lease)?;
