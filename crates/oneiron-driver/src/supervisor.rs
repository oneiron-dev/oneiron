...
use oneiron::{
    BudgetExhaustionPolicy, BudgetGuard, CommitmentWakeExecutor, CommitmentWakeProposalPlanner,
    ConsolidationExecutor, ConsolidationSink, DEFAULT_DREAMER_CHILD_RESERVE_UNITS,
    DREAMER_GRACEFUL_WRAP_WINDOW_MS, DREAMER_WAKE_PASS_WALL_CLOCK_CEILING_MS,
    DreamerAttemptExecutor, DreamerClaimAuthoringStrategy, DreamerConsolidationScope,
...
use oneiron_server::{
    ...
    voice_host::{VoiceHost, VoiceHostBindings, VoiceHostConfig},
};
...
impl PassExecutorFactory for ConsolidationExecutorFactory {
    ...
    fn voice_host(&self, vault: &Vault, guard: &BudgetGuard) -> Result<Option<VoiceHost>> {
        ...
        .map(Some)
        .map_err(|_| oneiron::Error::InvalidConfig("voice attachment refused".into()))
    }
...
async fn run_one_pass<F: PassExecutorFactory>(
    ...
    // Retain the optional host for this pass only. No stream is invented and
    // no serve task is started: the lifecycle owner still owes those bindings.
    #[cfg(all(unix, feature = "voice"))]
    let _voice_host = factory
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // CMT-3 (ONE-1540): the commitment-wake composition site
    // -----------------------------------------------------------------------
    ...
    fn admit(vault: &Vault, now: u64) -> DreamerAdmittedAttempt {
        ...
        let outcome = store
            .admit_next_consolidation(oneiron::dreamer_runner::AdmitDreamerConsolidationAttempt {
                ...
                claim_authoring_tier:
                    oneiron::dreamer_runner::DreamerClaimAuthoringBatchTier::batch(),
                claim_authoring:
                    oneiron::dreamer_runner::DreamerClaimAuthoringAdmission::single_pass(),
    ...
    mod voice {
        ...
        use oneiron::voice_cascade::{AsrUpdate, VoiceSessionConfig};
        use oneiron_server::runtime::RuntimeConfig;
        use oneiron_server::voice_host::HostError;
        ...
        fn voice_config(vault: &Arc<Vault>) -> VoiceHostConfig {
            ...
            VoiceHostConfig {
                ...
                runtime,
                extraction_prompt: "Extract entity_labels and salient_terms as JSON.".to_owned(),
                session: VoiceSessionConfig::new("driver-voice-test", InterlocutorResolutionInput {
                    owner_session: false,
                    parties: Vec::new(),
        ...
        async fn configured_voice_is_attached_after_guard_and_fails_closed() {
            ...
            assert!(matches!(result, Err(PassRunError::PreAdmission(_))));
