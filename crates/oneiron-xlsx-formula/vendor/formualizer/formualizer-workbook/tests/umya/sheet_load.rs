//! Regression tests for issue #332 on the umya backend — the backend behind
//! `recalculate_file`, the save/round-trip path and the Python `.xlsx` write
//! path. A freshly constructed engine is seeded with a default sheet, and the
//! loader used to append the file's sheets alongside it, leaving a phantom
//! `Sheet1` and shifting every `SHEET()` result by one.
...
#[test]
fn umya_load_folds_default_sheet_into_first_file_sheet() {
...
#[test]
fn umya_load_file_containing_sheet1_has_no_duplicate() {
...
#[test]
fn umya_load_single_renamed_sheet() {
    let path = build_workbook(|book| {
        book.get_sheet_by_name_mut("Sheet1")
            .unwrap()
            .set_name("Report");
...
#[test]
fn umya_load_into_non_fresh_engine_preserves_user_data() {
    let path = data_extra_fixture();
    let mut backend = UmyaAdapter::open_path(&path).unwrap();
    let mut engine: Engine<_> = Engine::new(TestWorkbook::new(), EvalConfig::default());
    engine
