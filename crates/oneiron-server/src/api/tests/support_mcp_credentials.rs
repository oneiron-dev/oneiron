...
use oneiron::authority::{CapabilitySlip, HostSlipIssuer, PairingPrincipal, pairing_binding_transcript};
use oneiron::federation::{Scope, ScopeAxis, ScopeId};

fn cache_key(label: &str) -> String {
    format!("test:mcp:paired:{}", blake3::hash(label.as_bytes()).to_hex())
}
fn holder_key(label: &str) -> SigningKey {
    SigningKey::from_bytes(&blake3::derive_key("oneiron/test/mcp-paired-holder/v2", label.as_bytes()))
}
...
pub(super) fn pair_mcp_credential(
    ...
    let issuer = HostSlipIssuer::from_secret(server.config.auth_secret.as_ref().unwrap().as_bytes()).unwrap();
    ...
    let link = server.vault().issue_pairing_link_for_principal(&issuer, authority, 3600,
        PairingPrincipal { holder_ref: Some(holder.clone()), actor_class: Some(class.gate_actor_class().to_owned()), org_ref: None }).unwrap();
    ...
    let slip = server.vault().redeem_pairing_link(&issuer, &link.ticket, &holder, binding, &signature.to_bytes()).unwrap();
    let token = slip.to_token().unwrap();
    server.vault().sync_state_put(&cache_key(label), token.as_bytes()).unwrap();
    token
}

pub(super) fn mcp_registered_credential(server: &SyncServer, label: &str) -> String {
    server.vault().sync_state_get(&cache_key(label)).unwrap()
        .map(|bytes| String::from_utf8(bytes).unwrap()).unwrap_or_else(|| label.to_owned())
}

pub(super) fn bind_mcp_request(server: &SyncServer, request: Request<Body>) -> Request<Body> {
    let Some(label) = request.headers().get(AUTHORIZATION)
        .and_then(|value| value.to_str().ok())
        .and_then(|value| value.strip_prefix("Bearer ")) else { return request; };
    let Some(token) = server.vault().sync_state_get(&cache_key(label)).unwrap() else { return request; };
    let key = holder_key(label);
