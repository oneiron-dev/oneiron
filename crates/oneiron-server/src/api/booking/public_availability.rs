...
fn query(input: &BookingAvailabilityInput, authority: &PublicBookingAuthority) -> serde_json::Value {
...
pub(super) fn validate_public_request(publication: &BookingPagePublication, request: &BookingOperationRequest) -> Result<(), ApiError> {
    let (event, has_constraint) = match request {
        BookingOperationRequest::Availability(input) => (&input.event_type, input.constraint.is_some()),
        BookingOperationRequest::Book(BookingBookInput::Hold(input)) => (&input.event_type, input.constraint.is_some()),
        _ => return Ok(()),
    };
    if !publication.event_types.iter().any(|card| card.key == *event)
...
pub(super) fn recheck(server: &SyncServer, authority: Option<&PublicBookingAuthority>) -> Result<(), ApiError> {
    if let Some(authority) = authority {
        let current = oneiron::booking::load_public_booking_page(&server.vault, authority.page_ref, now_secs()?)
            .map_err(|_| public::public_booking_not_found())?;
        if current.as_ref() != Some(&authority.publication) { return Err(public::public_booking_not_found()); }
    }
    Ok(())
}

pub(super) fn cached_response(
    server: &SyncServer, page: EntityId, request: &BookingOperationRequest, authority: Option<&PublicBookingAuthority>,
) -> Result<Option<BookingOperationResponse>, ApiError> {
    let (BookingOperationRequest::Availability(input), Some(authority)) = (request, authority) else { return Ok(None); };
    let Some(bytes) = cached_slot_list_body(server, &page, Some(&input.event_type))? else { return Ok(None); };
    let Ok(cached) = serde_json::from_slice::<CachedAvailability>(&bytes) else { return Ok(None); };
    if cached.query != query(input, authority) { return Ok(None); }
    if !matches!(&cached.response, BookingOperationResponse::Availability { .. }) { return Ok(None); }
    Ok(Some(cached.response))
}

pub(super) fn remember_response(
    server: &SyncServer, page: EntityId, input: &BookingAvailabilityInput, authority: Option<&PublicBookingAuthority>, response: &BookingOperationResponse,
) -> Result<(), ApiError> {
    let Some(authority) = authority else { return Ok(()); };
    let bytes = serde_json::to_vec(&CachedAvailability { query: query(input, authority), response: response.clone() })
...
pub(super) fn solve(
    server: &Arc<SyncServer>, page_ref: EntityId, input: BookingAvailabilityInput,
    now: u64, public_authority: Option<&PublicBookingAuthority>,
