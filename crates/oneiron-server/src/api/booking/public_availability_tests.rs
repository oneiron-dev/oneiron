...
fn listing(now: u64) -> BookingAvailabilityInput {
    BookingAvailabilityInput {
        ...
        constraint: None,
        session_ref: "availability".to_owned(),
    }
}
...
async fn public_availability_serves_each_card_and_changed_timezone_window_constraint() {
    ...
    vault
        .put_claim(&id(0x75), &body, TimeRange { start: 1, end: 1 }, 1)
    ...
    for key in ["intro", "second"] {
        ...
        let response = fixture
            .route(
                "POST",
                &format!("/public/booking/{}/availability", fixture.token),
                serde_json::to_value(input).expect("request"),
            )
            .await;
        assert_eq!(response.status(), StatusCode::OK);
        assert_inline(&response);
        let result: BookingOperationResponse =
            serde_json::from_slice(&bytes(response).await).expect("response");
        let BookingOperationResponse::Availability { slots, .. } = result else {
            panic!("availability");
        };
        assert!(!slots.is_empty() && slots.len() <= 128);
        assert!(slots.iter().all(
            |slot| slot.end_utc - slot.start_utc == if key == "intro" { 1_800 } else { 3_600 }
        ));
    }
    let mut input = listing(now);
...
async fn public_slot_cache_reuses_matching_query_but_not_changed_window_or_revocation() {
    for _ in 0..3 {
        ...
        assert_eq!(
            second.status(),
            StatusCode::OK,
            "a matching cache hit must not spend a second token"
        );
        ...
        let miss = fixture
            .route(
                "POST",
                &path,
                serde_json::to_value(changed).expect("request"),
            )
            .await;
        assert_ne!(
            miss.status(),
            StatusCode::OK,
            "a different query cannot reuse the scope's cached body or bypass quota"
        );
        ...
        let revoked = fixture
            .route(
                "POST",
                &path,
                serde_json::to_value(listing(now)).expect("request"),
            )
            .await;
        assert_eq!(
            revoked.status(),
            StatusCode::NOT_FOUND,
            "cached slots are not publication authority"
        );
        return;
    }
...
async fn queued_public_hold_loses_authority_before_the_lifecycle_writer() {
    ...
    let authority = oneiron::booking::publication::PublicBookingAuthority {
        page_ref: fixture.page,
        publication: load_public_booking_page(&fixture.server.vault, fixture.page, now)
            .expect("read")
            .expect("public"),
    };
    oneiron::booking::lifecycle::enqueue_booking_verb_with_publication(
        ...
        BookingVerbRequest::Hold(HoldSpec {
            ...
            session_key: session_key(fixture.page, "queued"),
            visitor_tz: "UTC".to_owned(),
            constraint: None,
            lease: HoldLeaseSpec::Ordinary,
            idempotency_key: Some("queued-public".to_owned()),
        }),
        now,
        Some(authority),
    )
    .expect("enqueue");
    fixture.publish_page(fixture.page, false, 1, now + 86_400);
    let outcome = oneiron::booking::run_booking_lifecycle_once(
        &fixture.server.vault,
        |_| Ok(EmptyOracle),
        &oneiron::booking::BookingLifecycleConsumerInput {
            local_node_id: 9,
            lease_owner: "public-regression".to_owned(),
            now_utc: now,
        },
    );
    assert!(
        outcome.is_err(),
        "delayed public attempts must recheck authority inside the writer"
    );
}
