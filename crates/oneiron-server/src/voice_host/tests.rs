...
async fn real_admission_exact_bounded_request_empty_signature_and_no_locks_across_await() {
    ...
    let (result, ()) = tokio::join!(work.run(), async {
        ...
        assert!(
            host.state.try_lock().is_ok(),
            "session guard must be released during provider await"
        );
        ...
        let ContentPart::Text { text: input } = &call.request.messages[1].content[0] else {
            panic!("text input")
        };
        assert_eq!(
        ...
        let ResponseFormat::Json { schema } = &call.request.envelope.response_format else {
            panic!("JSON schema")
        };
    ...
    let AsrUpdate::Partial(partial) = result.unwrap() else {
        panic!("partial")
    };
    assert_eq!(
        partial.decision,
    ...
    assert!(
        host.prepare(&token, 1, text.to_owned(), false).is_err(),
        "successful observation consumed once"
    );
}
...
async fn later_revision_prepared_while_backend_waits_rejects_old_result_before_effects() {
    ...
    let (old_result, ()) = tokio::join!(old.run(), async {
        ...
        call.reply
            .send(Ok(response(
                json!({"entity_labels": ["old"], "salient_terms": ["old"]}),
            )))
            .unwrap();
    });
    ...
    assert_eq!(
        ...
        7,
        "stale provider work is still charged"
    );
    let (result, ()) = tokio::join!(newer.take().unwrap().run(), async {
        let call = calls.recv().await.unwrap();
        let ContentPart::Text { text } = &call.request.messages[1].content[0] else {
            panic!("input")
        };
        assert_eq!(
    ...
    let AsrUpdate::Final(request) = result.unwrap() else {
        panic!("new final")
    };
...
async fn close_and_reopen_same_label_during_provider_wait_drops_late_final() {
    ...
    assert_eq!(budget.read().reserved_units, 0);
    assert!(
        host.prepare(&replacement, 1, "new".to_owned(), true)
            .is_ok()
    );
}
...
async fn budget_denial_and_provider_error_preserve_exact_revision_for_retry() {
    ...
    let denied = host
        .prepare(&token, 1, "final".to_owned(), true)
        .unwrap()
        .run()
        .await;
    assert!(matches!(
    ...
    let (result, ()) = tokio::join!(failed.run(), async {
        let call = calls.recv().await.unwrap();
        assert_eq!(
            call.lease_id, "voice-test:metered:2",
            "denial issued no lease"
        );
        call.reply
            .send(Err(RetryableLlmError::ServerError.into()))
            .unwrap();
    });
    assert!(matches!(
        result,
        Err(HostError::Llm(LlmError::Retryable(
            RetryableLlmError::ServerError
        )))
    ));
    assert_eq!(budget.read().reserved_units, 0);
...
async fn dropped_future_and_shutdown_release_reservation_without_consuming_observation() {
    ...
    let mut work = Box::pin(
        host.prepare(&token, 1, "pending".to_owned(), true)
            .unwrap()
            .run(),
    );
    ...
    drop(work);
    assert!(
        call.reply.send(Ok(empty())).is_err(),
        "provider receiver was cancelled"
    );
    assert_eq!(budget.read().reserved_units, 0);
...
async fn malformed_truncated_and_injected_responses_never_retrieve() {
    for value in [
        ...
        assert_eq!(
            budget.read().used_units,
            7,
            "malformed provider work still settles"
        );
        assert_eq!(budget.read().reserved_units, 0);
