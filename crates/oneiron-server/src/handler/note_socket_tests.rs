...
async fn authenticated_note_socket_preserves_pins_provenance_and_review_after_reopen() {
    ...
    let (mut client, _events) = SyncClient::new(
        ...
        SyncClientConfig {
            ...
            note_session: Some(oneiron::sync::client::NoteSyncSession::new(token)),
            ..Default::default()
        },
    )
    .unwrap();
...
async fn document_handler_refuses_unbound_and_selector_impersonation_and_raw_pin_injection() {
    ...
    let bind = |actor: EntityId| {
        ...
        crate::auth::CoreAuth::from_bind_token(&token, &server.config, vault.as_ref()).unwrap()
    };
    ...
    assert!(
        super::documents::document_delivery(&server, &state, &receipt_frame)
            .unwrap()
            .is_empty()
    );
