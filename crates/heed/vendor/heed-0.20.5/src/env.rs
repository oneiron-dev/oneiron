...
impl EnvOpenOptions {
    ...
    pub unsafe fn open<P: AsRef<Path>>(&self, path: P) -> Result<Env> {
        let lock = OPENED_ENV.write().unwrap();
        ...
        // ONEIRON FORK (ONE-218), see crates/heed/PROVENANCE.md: the ordinary
        // open is upstream behaviour expressed through the shared seam below.
        // The canonicalized pathname is BOTH the `OPENED_ENV` cache identity
        // and the exact byte string handed to `mdb_env_open`, and neither
        // caller hook does anything, so every existing caller is unaffected.
        let cache_identity = path.clone();
        unsafe { self.open_locked(lock, cache_identity, &path, || {}, || {}) }
    }

    /// ONEIRON FORK (ONE-218) — the descriptor-stable open seam.
    ///
    /// Opens the environment at EXACTLY `open_path`, byte for byte, while
    /// registering it in heed's process-global environment map under the
    /// separate `cache_identity` key.
    ///
    /// `open_path` is turned straight into a [`CString`] and handed to the one
    /// `mdb_env_open` call this crate makes. It is never canonicalized,
    /// re-resolved, joined, or substituted by `cache_identity`. A caller that
    /// passes a capability path such as `/proc/self/fd/<dirfd>` therefore keeps
    /// LMDB's own opens of `data.mdb` and `lock.mdb` pinned to the directory
    /// that descriptor holds: a rename, swap, or ABA of an ordinary pathname
    /// cannot redirect them, because no ordinary pathname is ever dereferenced.
    ///
    /// `cache_identity` is bookkeeping only — the single-`Env`-per-identity
    /// cache lookup, [`Env::path`], the [`Env::prepare_for_closing`] and drop
    /// de-registration, and [`env_closing_event`]. A caller opening through a
    /// volatile path (a `/proc/self/fd` entry is valid only while its
    /// descriptor lives) is expected to pass the stable identity of the root
    /// instead. It never reaches LMDB.
    ///
    /// `before_open` runs after every path, option, and cache preparation step,
    /// IMMEDIATELY before `mdb_env_open`. `after_open` runs IMMEDIATELY after
    /// that call returns, before the result is inspected and before the `Env`
    /// is published. They exist so a caller can prove what the final
    /// dereference does; both are no-ops on the ordinary [`Self::open`] path.
    /// Neither runs when the cache already holds `cache_identity`, because that
    /// returns the existing `Env` without calling into LMDB at all.
    ///
    /// # Safety
    ///
    /// Every safety requirement of [`Self::open`] applies unchanged. In
    /// addition:
    ///
    /// - `open_path` must name, for the whole duration of this call, the
    ///   directory (or file, under [`EnvFlags::NO_SUB_DIR`]) the caller intends
    ///   LMDB to open. When it is a `/proc/self/fd/<fd>` path, the caller must
    ///   keep `fd` open at least until this call returns: LMDB resolves that
    ///   path again while opening `lock.mdb`, `data.mdb`, and its synchronous
    ///   meta descriptor, all inside `mdb_env_open`.
    /// - `cache_identity` must identify that same environment one-to-one. Two
    ///   live environments must never share an identity, and one environment
    ///   must not be opened twice under two identities; heed cannot check a
    ///   caller-supplied identity, and LMDB's single-`Env`-per-environment rule
    ///   still holds.
    /// - `before_open` and `after_open` must not call back into heed — this
    ///   thread holds the environment-map write lock while they run — and must
    ///   not unwind across the surrounding LMDB call.
    ...
    {
        let lock = OPENED_ENV.write().unwrap();
        unsafe { self.open_locked(lock, cache_identity, open_path, before_open, after_open) }
    }

    /// ONEIRON FORK (ONE-218): the single environment-construction body shared
    /// by [`Self::open`] and [`Self::open_with_cache_identity`] — one
    /// `mdb_env_create`, one option setup, ONE `mdb_env_open` call site, one
    /// cache insertion. There is deliberately no second implementation.
    ///
    /// # Safety
    ///
    /// See [`Self::open`] and [`Self::open_with_cache_identity`]: this is their
    /// shared body and carries exactly their contract.
    unsafe fn open_locked<B, A>(
        &self,
        mut lock: RwLockWriteGuard<'_, HashMap<PathBuf, EnvEntry>>,
        cache_identity: PathBuf,
        open_path: &Path,
        before_open: B,
        after_open: A,
    ...
    where
        ...
        A: FnOnce(),
    {
        match lock.entry(cache_identity) {
            ...
            Entry::Vacant(entry) => {
                // ONEIRON FORK (ONE-218): the EXACT open path, never
                // `entry.key()`. The cache identity and the byte string LMDB
                // dereferences are separable here, and only this string is
                // allowed to reach the C call below.
                let path_str = CString::new(open_path.as_os_str().as_bytes()).unwrap();
                ...
                unsafe {
                    ...
                    // ONEIRON FORK (ONE-218): every path, option, and cache
                    // preparation step is done. `before_open` fires HERE, at
                    // the true final dereference, and `after_open` fires the
