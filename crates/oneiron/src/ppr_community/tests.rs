...
fn edge(a: u32, b: u32, kind: EdgeKind) -> CommunityEdge {
    CommunityEdge {
        ...
        kind,
        deleted: false,
        value: DecodedEdgeValue {
            layout: EdgeValueLayout::SemanticBare,
            weight: 0.25,
            created_at: 1,
            vad: None,
            provenance: None,
...
fn experiment() -> PprCommunityConfig {
    PprCommunityConfig {
        beta: PPR_COMMUNITY_BETA_EXPERIMENT,
        ..Default::default()
    }
}

fn scored(n: u32, score: f32) -> ScoredEntity {
    ScoredEntity { id: id(n), score }
}
...
fn projection_excludes_deleted_retracted_zero_self_and_absent_endpoints() {
    ...
    let mut zero = edge(1, 2, EdgeKind::BelongsTo);
    zero.value.weight = 0.0;
    let p = project_graph(
        &[id(1), id(2)],
        &[
            deleted,
            retracted,
            zero,
            edge(1, 1, EdgeKind::Supports),
            edge(1, 3, EdgeKind::Supports),
        ],
    )
    .expect("projection");
    assert!(p.edges.is_empty());
    assert_eq!(p.entities, vec![id(1), id(2)]);
}
...
fn projection_sums_distinct_directed_relations_and_deduplicates_records() {
    ...
    let p = project_graph(
        ...
        &[
            a,
            a,
            edge(1, 2, EdgeKind::Mentions),
            edge(1, 2, EdgeKind::Supports),
        ],
    )
    .expect("projection");
    ...
    assert_eq!(
        project_graph(&[id(1), id(2)], &[a, conflict]),
        Err(CommunityError::Graph)
    );
    for bad in [f32::NAN, f32::INFINITY, -1.0, 1.1] {
        let mut bad_edge = a;
        bad_edge.value.weight = bad;
...
fn assert_connected(snapshot: &CommunitySnapshot, projection: &CommunityProjection) {
    for members in snapshot.members.values() {
        ...
        let mut seen = BTreeSet::from([members[0]]);
...
fn leiden_finds_dense_blocks_and_is_permutation_deterministic_at_both_levels() {
    ...
    assert_eq!(
        expected.nodes[&id(7)].fine,
        CommunityId::from_members(&[id(7)]).expect("singleton")
    );
    assert_connected(
        &expected,
        &project_graph(&entities, &edges).expect("projection"),
    );
    for shift in 0..12 {
        ...
        assert_eq!(
            actual.encode_rows().expect("rows"),
            expected.encode_rows().expect("rows")
        );
    }
}
...
fn leiden_refinement_restarts_singletons_and_requires_gamma_connected_subsets() {
    let p = project_graph(
        &[id(1), id(2), id(3)],
        &[edge(1, 2, EdgeKind::About), edge(2, 3, EdgeKind::About)],
    )
    .expect("projection");
    ...
    assert_eq!(
        refine_partition(&graph, &[0, 0, 0]),
        vec![vec![0], vec![1], vec![2]]
    );
    assert_eq!(
        connected_groups(&graph, &[0, 1, 0]),
        vec![vec![0], vec![1], vec![2]]
    );
}
...
fn quality(graph: &Graph, labels: &[usize]) -> i128 {
    let internal: u64 = graph
        ...
        .map(|(v, edges)| {
            edges
                .iter()
                .filter(|&(&u, _)| u > v && labels[u] == labels[v])
                .map(|(_, &w)| w)
                .sum::<u64>()
        })
        .sum();
...
fn quotient_keeps_vertex_mass_and_cpm_deltas() {
    ...
    edges.push(edge(4, 5, EdgeKind::About));
...
fn incremental_refresh_matches_full_and_preserves_unaffected_membership() {
    ...
    let (incremental, report) = refresh(
        &entities,
        &edges,
        Some(&old),
        &[id(1), id(2), id(3), id(1)],
    ...
    assert_eq!(report.changed_entities, 3);
    assert_eq!(report.recomputed_entities, 3);
...
fn frontier_closes_new_connections_and_full_fallback_is_strictly_above_five_percent() {
    ...
    assert_eq!(report.recomputed_entities, 6);
    assert_eq!(new, refresh(&entities, &edges, None, &[], 2).0);
    assert!(
        !refresh(
            &entities,
            &edges,
    ...
    assert!(
        refresh(&entities, &edges, Some(&old), &[], 2)
            .1
            .full_recompute
    );
...
fn activation_covers_single_dominant_repeated_top_five_and_boundaries() {
    ...
    assert!(
        activated_communities(&cache, &[])
            .expect("active")
            .is_empty()
    );
    assert_eq!(
        activated_communities(&cache, &[scored(1, 1.0)]).expect("single"),
        BTreeSet::from([fine])
    );
    assert!(
        activated_communities(&cache, &[scored(1, 1.49), scored(9, 1.0)])
            .expect("not dominant")
            .is_empty()
    );
    assert_eq!(
        activated_communities(&cache, &[scored(1, 1.5), scored(9, 1.0)]).expect("dominant"),
        BTreeSet::from([fine])
    );
    assert_eq!(
        activated_communities(&cache, &[scored(1, 1.0), scored(9, 0.9), scored(2, 0.8)])
            .expect("shared"),
        BTreeSet::from([fine])
    );
    assert!(
    ...
    assert!(
        activated_communities(&cache, &[scored(999, 1.0)])
            .expect("missing")
            .is_empty()
    );
}
...
fn multiplier_obeys_size_formula_decay_cap_and_large_community_disable() {
    ...
    assert!(
        (f64::from(decayed)
            - (1.0 + (expected - 1.0) * (-f64::from(PPR_COMMUNITY_USAGE_DECAY) * 10.0).exp()))
        .abs()
            < 1e-7
    );
    ...
    assert_eq!(
        community_multiplier(1, 100, u32::MAX, &c).expect("usage"),
        1.0
    );
    assert_eq!(
...
fn beta_zero_is_bit_exact_even_with_bad_evidence_zero_limit_and_invalid_other_config() {
    ...
    let mut scores = vec![
        scored(2, -0.0),
        scored(1, f32::from_bits(0x7fc00187)),
        scored(2, f32::INFINITY),
    ];
    let before: Vec<_> = scores.iter().map(|s| (s.id, s.score.to_bits())).collect();
    for beta in [0.0, -0.0] {
        let context = CommunityBoostContext {
            ordered_seeds: &[scored(1, f32::NAN)],
            result_limit: 0,
            session_usage: &usage,
        };
        let config = PprCommunityConfig {
            beta,
            gamma: f32::NAN,
            ..Default::default()
        };
        assert_eq!(
            apply_community_prior(&mut scores, &cache, &context, &config).expect("bypass"),
            CommunityBoostReport::default()
        );
        assert_eq!(
            scores
                .iter()
    ...
    assert_ne!(
        community_cache_identity(0.2, 7).expect("key"),
        community_cache_identity(0.2, 8).expect("key")
    );
    assert_ne!(
        community_cache_identity(0.2, 7).expect("key"),
        community_cache_identity(0.3, 7).expect("key")
    );
}
...
fn prior_boosts_matching_fine_not_merely_coarse_and_errors_are_atomic() {
    ...
    assert_eq!(report.boosted_candidates, 1);
    assert_eq!(report.activated_communities, 1);
    assert!(
        scores
            .iter()
            .find(|s| s.id == id(2))
    ...
    let mut overflow = vec![scored(2, f32::MAX), scored(9, 1.0)];
...
fn mmr_breaks_ties_by_fine_then_coarse_novelty_and_ids() {
    let pool = vec![
        ranked(1, 1, 1, 1.0, true),
        ranked(2, 1, 1, 1.0, true),
        ranked(3, 2, 1, 1.0, true),
        ranked(4, 3, 2, 1.0, true),
    ];
    let rows = diversify(pool.clone(), 4, PPR_COMMUNITY_MAX_TOP_K_FRACTION);
    assert_eq!(
        rows.iter().map(|r| r.entity.id).collect::<Vec<_>>(),
        vec![id(1), id(4), id(3), id(2)]
    );
    let reversed = diversify(
        pool.into_iter().rev().collect(),
        4,
        PPR_COMMUNITY_MAX_TOP_K_FRACTION,
    );
    assert_eq!(
        rows.iter().map(|r| r.entity.id).collect::<Vec<_>>(),
...
fn small_graph_family_keeps_connected_nested_partitions_and_stable_ids() {
    ...
    for mask in 0_u32..64 {
        ...
        for a in 1..=8 {
            for b in a + 1..=8 {
                if (mask >> ((a + b) % 6)) & 1 != 0 {
                    ...
                    if (a * b + mask) % 3 == 0 {
                        edges.push(edge(a, b, EdgeKind::Supports));
                    }
                }
            }
        }
        ...
        snapshot
            .validate(1)
            .expect("nested, complete, content-addressed partition");
        assert_connected(
            &snapshot,
            &project_graph(&entities, &edges).expect("projection"),
        );
...
fn prior_consumes_session_usage_and_no_activation_leaves_the_input_untouched() {
    ...
    assert_eq!(
        scores
            ...
            .score
            .to_bits(),
        community_multiplier(8, 100, 10, &experiment())
            .expect("multiplier")
            .to_bits()
    );
    let inactive = CommunityBoostContext {
        ordered_seeds: &[scored(1, 1.0), scored(9, 0.9)],
        result_limit: 0,
        session_usage: &usage,
    };
    let before = scores.clone();
    assert_eq!(
        apply_community_prior(&mut scores, &cache, &inactive, &experiment()).expect("inactive"),
        CommunityBoostReport::default()
    );
...
fn incremental_refresh_detects_added_entities_without_a_separate_entity_frontier_entry() {
    ...
    let (old, _) = refresh(&entities, &edges, None, &[], 1);
    let mut added = entities;
    added.push(id(101));
    let mut changed_edges = edges;
    changed_edges.push(edge(3, 101, EdgeKind::About));
...
fn ordered_seed_evidence_survives_id_sort_and_does_not_fabricate_explicit_scores() {
    let ordered = ordered_seed_evidence(
        &[id(1), id(9), id(2), id(1)],
        &[scored(9, 1.0), scored(1, 1.5)],
    )
    .expect("evidence");
    assert_eq!(
        ordered,
        vec![scored(1, 1.5), scored(9, 1.0), scored(2, 0.0)]
    );
    let snapshot = fixture();
    let cache = PprCommunityCache::new(&snapshot, 7).expect("cache");
    assert!(
        activated_communities(&cache, &[scored(1, 0.0), scored(9, 0.0)])
            .expect("no evidence")
            .is_empty()
    );
...
fn deferred_diversity_uses_only_final_pool_and_keeps_fused_score_bits() {
    ...
    let (report, boosted) =
        boost_community_scores(&mut scores, &cache, &context, &experiment()).expect("prior");
    assert_eq!(report.boosted_candidates, 7);
    assert_eq!(
        scores.len(),
        9,
        "expansion must not truncate before filters and fusion"
    );
    scores.retain(|row| row.id != id(9) && row.id != id(17));
    scores.extend((40..=44).map(|n| scored(n, 0.5))); // other fused channels
    let before: BTreeMap<_, _> = scores
        .iter()
        .map(|row| (row.id, row.score.to_bits()))
        .collect();
    let report = apply_community_diversity(&mut scores, &cache, &boosted, 10, &experiment())
        .expect("final diversity");
    assert_eq!(scores.len(), 10);
    assert_eq!(
        scores
            .iter()
            .filter(|row| boosted.contains(&row.id))
            .count(),
        7
    );
    for row in &scores {
