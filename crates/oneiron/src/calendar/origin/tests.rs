...
#[test]
fn late_origin_binding_preserves_source_invalidation_without_hiding_native_events() {
    for origin in [
        CalendarOrigin::Dreamer,
        CalendarOrigin::Native,
        CalendarOrigin::Imported,
    ] {
        let (dir, vault, _actor) = fixture();
        let turn = EntityId::now();
        vault
            .put_entity(&turn, ENTITY_TYPE_TURN, time(), 1, b"turn")
            .unwrap();
        let mut pending = CalendarEventInput {
            origin: Some(origin),
            ..input()
        };
        if origin == CalendarOrigin::Dreamer {
            pending.evidence_turn_ids = vec![turn];
            pending.source_frontiers = vec!["v1".into()];
        } else if origin == CalendarOrigin::Imported {
            pending.import_source = Some("feed".into());
            pending.external_id = Some("uid".into());
        }
        let event = EntityId::now();
        let body = pending.encode().unwrap();
        vault
            .batch()
            .put_replicated(&event, ENTITY_TYPE_EVENT, time(), 1, &body)
            .commit()
            .unwrap();
        vault
            .put_edge(&event, EdgeKind::DerivedFrom, &turn, 1.0)
            .unwrap();
        let request = CalendarReadRequest {
            event_ref: event.to_hex(),
        };
        assert!(read_event(&vault, &request).unwrap().is_none());
        ...
        vault
            ...
            .unwrap();
        assert_eq!(vault.calendar_event_origin(event).unwrap(), origin);
        assert_eq!(
            vault.get(&event).unwrap().is_some(),
            origin != CalendarOrigin::Dreamer
        );
        assert_eq!(
            read_event(&vault, &request).unwrap().is_some(),
            origin != CalendarOrigin::Dreamer
        );

        // The deletion fact is durable, not just the temporary unbound gate.
        drop(vault);
        let reopened = Vault::open(dir.path(), crate::VaultConfig::default()).unwrap();
        assert_eq!(
            read_event(&reopened, &request).unwrap().is_some(),
            origin != CalendarOrigin::Dreamer
        );
    }
}
