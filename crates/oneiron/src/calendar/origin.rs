...
/// Dreamer while that binding is incomplete.
pub(crate) fn replay_origin_bound(
    ...
    let Some(origin) = declared_origin(&row.body)? else {
        return Ok(true);
    };
    Ok(live_origin(store, txn, event)? == Some(origin))
}

fn declared_origin(body: &[u8]) -> Result<Option<CalendarOrigin>> {
    let mut input = body;
...
pub(crate) fn survives_source_deletion(
    ...
    if !replay_origin_bound(store, txn, event)?
        && let Some(row) = store.port_entity_record(txn, &event)?
    {
        // A declared Dreamer is derived even before its claim binds. Preserve
        // the pending native/imported behavior without delaying invalidation
        // of ordinary reads and retrieval indexes for extracted events.
        return Ok(declared_origin(&row.body)? != Some(CalendarOrigin::Dreamer));
    }
...
pub(crate) fn invalidate_dependents(
    ...
    for entry in vault.store.port_edges(
        ...
        if EntityMetadataHeader::parse(&raw).is_some_and(|h| h.entity_type == ENTITY_TYPE_EVENT) {
            // A delayed claim must not erase a source-deletion fact. Keep the
            // typed row until binding completes, but remember invalidation for
            // its declared Dreamer origin. Pending native/imported rows must
            // not inherit the legacy Dreamer default.
            let origin = match live_origin(&vault.store, txn, event)? {
                Some(origin) => origin,
                None => declared_origin(&raw[ENTITY_METADATA_HEADER_LEN..])?
                    .unwrap_or(CalendarOrigin::Dreamer),
            };
            if origin == CalendarOrigin::Dreamer {
