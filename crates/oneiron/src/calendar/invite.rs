...
mod tests {
    ...
    fn faceted_sender_selection_preserves_actor_custody_and_ambiguity_checks() -> crate::Result<()>
    {
        use crate::channel_identity::{
            ChannelIdentityFulfillment, DelegatedGrant, DelegatedGrantScope,
        ...
        assert!(
            sending_identity(&vault, actor)
                .expect("sender lookup")
                .is_none()
        );

        let grant =
            DelegatedGrant::new("oauth/faceted-sender", vec![DelegatedGrantScope::MailRead]);
        ...
        let email = sending_identity(&vault, actor)
            .expect("lookup")
            .expect("faceted email");
        assert_eq!(email.binding, binding);
        assert_eq!(email.address_or_handle, "sender-169@example.com");
        assert_eq!(
            primary_calendar_domain(&vault, actor).expect("domain"),
        ...
        assert!(
            sending_identity(&vault, actor).is_err(),
        ...
        let calendar = sending_identity(&vault, actor)
            .expect("lookup")
            .expect("faceted calendar");
        assert_eq!(calendar.binding, binding);
        assert_eq!(calendar.address_or_handle, "sender-171@example.com");
        put(
            0xAC,
            "calendar",
