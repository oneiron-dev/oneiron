...
fn word(start_ms: u64, end_ms: u64, text: &str) -> TranscriptWord {
    TranscriptWord {
        word_id: format!("word-{start_ms}"), pack_id: "pack-1".into(),
        start_ms, end_ms, text: text.into(), confidence: None,
        speaker_cluster: "unassigned".into(),
    }
}

fn track(start_ms: u64, end_ms: u64, speaker: &str) -> SpeakerTrack {
    SpeakerTrack { start_ms, end_ms, speaker_cluster: speaker.into() }
}

#[test]
fn boundary_packing_preserves_all_speech_without_duplicate_audio() {
    let spans = [span(1_000, 46_000), span(48_000, 93_000),
        span(95_000, 140_000), span(142_000, 187_000)];
    let packs = pack_speech(&spans, 190_000).unwrap();
    assert_eq!(packs.len(), 2);
    assert!(packs.iter().all(|p| (90_000..=120_000).contains(&p.audio_ms)));
    ...
    assert!(source.windows(2).all(|w| w[0].source_end_ms <= w[1].source_start_ms));
    for pack in &packs {
        ...
        assert!(pack.source_spans.windows(2).all(|w| w[0].pack_end_ms == w[1].pack_start_ms));
    }
}
...
fn short_tail_and_boundary_limited_pack_are_not_dropped() {
    let packs = pack_speech(&[span(0, 80_000), span(81_000, 161_000), span(162_000, 165_000)], 166_000).unwrap();
    assert_eq!(packs.len(), 2);
    assert!(packs.iter().all(|pack| pack.audio_ms < 90_000));
    assert_eq!(packs.iter().map(|pack| pack.speech_ms).sum::<u64>(), 163_000);
    assert_eq!(packs.last().unwrap().source_spans.last().unwrap().source_end_ms, 165_250);
}
...
fn overlapping_vad_spans_merge_and_nested_disorder_is_refused() {
    ...
    assert_eq!(pack_speech(&[span(100, 900), span(200, 400), span(150, 180)], 1_000), Err(AudioError::InvalidVad));
...
fn audio_slices_and_word_clock_reconstruct_source_after_removed_silence() {
    ...
    let pack = &packs[0];
    let audio = Pcm16 { samples: (0..35_200).map(|n| i16::try_from(n % 1_009).unwrap()).collect() };
    let packed = packed_audio(&audio, pack).unwrap();
    let expected: Vec<i16> = audio.samples[..7_200].iter()
        .chain(&audio.samples[28_000..35_200]).copied().collect();
    ...
    assert_eq!(source_times(pack, 449, 451), Err(AudioError::WordCrossesPackSeam));
...
fn alignment_refuses_track_overlap_or_uncovered_word_without_partial_mutation() {
    ...
    assert_eq!(align_words_to_speakers(&mut words, &[track(0, 20, "a")], 50),
        Err(AudioError::UnlabelledWord { word_id: "word-30".into() }));
    assert_eq!(words, before);
    for tracks in [vec![track(0, 20, "a"), track(19, 40, "b")], vec![track(0, 51, "a")], vec![track(0, 50, " ")]] {
        assert_eq!(align_words_to_speakers(&mut words, &tracks, 50), Err(AudioError::NonExclusiveTracks));
        assert_eq!(words, before);
    }
    let mut reversed = [word(30, 40, "late"), word(0, 10, "early")];
    assert_eq!(align_words_to_speakers(&mut reversed, &[track(0, 50, "a")], 50), Err(AudioError::InvalidWords));
}
...
fn cleanup_rejects_invention_deletion_reorder_symbol_changes_and_word_joining() {
    ...
    for (raw, cleaned) in [
        ...
        assert_eq!(validate_cleanup(raw, cleaned), Err(AudioError::CleanupInventedContent));
    }
    ...
    assert_eq!(apply_cleanup(&mut turns, vec!["Hello!".into(), "yes".into()]), Err(AudioError::CleanupInventedContent));
    assert_eq!(turns, before);
    assert_eq!(apply_cleanup(&mut turns, vec!["hello not".into()]), Err(AudioError::CleanupChangedTurns));
    assert_eq!(turns, before);
}
