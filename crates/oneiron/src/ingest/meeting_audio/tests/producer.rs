...
fn two_pack_fixture_labels_every_word_from_one_full_file_call_then_normalizes_after_one_consent() {
    ...
    assert_eq!(document["producer"]["batch_default"]["basis"], "provisional");
    assert_eq!(document["diarization"]["scope"], "full_file");
    assert_eq!(document["diarization"]["track_kind"], "exclusive");
    assert_eq!(document["diarization"]["provenance"]["invocation_id"], "global-call");
    assert_eq!(document["cleanup"]["status"], "turns_only");
    let labels: Vec<_> = document["words"].as_array().unwrap().iter()
        .map(|word| word["speaker_cluster"].as_str().unwrap()).collect();
    assert_eq!(labels, ["global-a", "global-b", "global-a", "global-b"]);
    let starts: Vec<_> = document["words"].as_array().unwrap().iter()
        .map(|word| word["start_ms"].as_u64().unwrap()).collect();
    ...
    let full_hash = document["recording"]["canonical_pcm_sha256"].as_str().unwrap();
    assert_eq!(host.requests[1], HostRequest::Vad { samples: 3_040_000, hash: full_hash.into() });
    assert_eq!(host.requests[2], HostRequest::Route {
        role: AsrRole::Asr, preferred: ProcessingTier::HomeFleet,
        local_only: false, model: "fixture-asr".into(),
    });
    for (index, request) in host.requests[3..5].iter().enumerate() {
        let HostRequest::Pack { id, samples, hash, glossary } = request else { panic!("expected pack call") };
        ...
        assert_eq!(document["producer"]["asr_packs"][index]["provenance"]["input_sha256"], hash.as_str());
    }
    assert_eq!(host.requests[5], HostRequest::Global { samples: 3_040_000, hash: full_hash.into() });
    assert_eq!(host.requests[6], HostRequest::Cleanup);

    let json_hash = sha256(artifact.json().as_bytes());
    let direct = INGEST_SOURCE_REGISTRY.normalize(MEETING_TRANSCRIPT_SOURCE_ID, artifact.json()).unwrap();
    let mut authorizer = Authorizer { response: ConsentResponse::Allow, requests: Vec::new() };
    ...
    assert_eq!(authorizer.requests[0].source_record_ids, ["turn-0001", "turn-0002", "turn-0003", "turn-0004"]);
    ...
    assert_eq!(direct.records.iter().map(|record| record.occurred_at).collect::<Vec<_>>(),
        [Some(1_001), Some(1_048), Some(1_095), Some(1_142)]);
    assert_eq!(direct.records.iter().map(|record| record.text.as_str()).collect::<Vec<_>>(),
    ...
    let config = INGEST_SOURCE_REGISTRY.get_config(MEETING_TRANSCRIPT_SOURCE_ID).unwrap();
    assert_eq!(KNOWN_INGEST_HARNESS_CONFIG.get_config(MEETING_TRANSCRIPT_SOURCE_ID), Some(config));
    assert_eq!(config.format, IngestSourceFormat::MeetingTranscriptV1);
    let skill = config.adapter_skill.unwrap();
    assert_eq!((skill.skill_id, skill.version), ("builtin.ingest.meeting-transcript", "1"));
...
fn host_contract_faults_fail_closed_with_typed_errors() {
    for (fault, expected) in [
        ...
        (Fault::MissingTrack, AudioError::UnlabelledWord { word_id: "word-000002".into() }),
...
fn local_only_is_passed_to_the_router_and_remote_fallback_is_refused() {
    let options = ProducerOptions { local_only: true, ..options() };
    let mut host = FixtureHost::small(Fault::RemoteRoute);
    assert_eq!(produce_meeting_transcript(&file(), &options, &mut host).unwrap_err(), AudioError::InvalidRoute);
    assert_eq!(host.requests.last().unwrap(), &HostRequest::Route {
        role: AsrRole::Asr, preferred: ProcessingTier::Local, local_only: true, model: "fixture-asr".into(),
    });
}
...
fn a_supplied_e1_reference_cannot_promote_fixture_execution_to_measured() {
    let options = ProducerOptions {
        batch_default: BatchDefault::MeasuredE1 { model_id: "fixture-asr".into(), evidence_ref: "unverified-host-reference".into() },
        ..options()
    };
    ...
    let document: Value = serde_json::from_str(artifact.json()).unwrap();
    assert_eq!(document["producer"]["execution"], "fixture");
    assert_eq!(document["producer"]["batch_default"]["evidence_ref"], "unverified-host-reference");
}

#[test]
fn bulk_consent_cannot_be_missing_or_rebound_to_other_artifacts_turns_or_vaults() {
    for response in [ConsentResponse::Missing, ConsentResponse::WrongScope,
        ConsentResponse::WrongHash, ConsentResponse::WrongTurns, ConsentResponse::BlankReceipt] {
        let mut host = FixtureHost::small(Fault::None);
        let artifact = produce_meeting_transcript(&file(), &options(), &mut host).unwrap();
        let mut authorizer = Authorizer { response, requests: Vec::new() };
        ...
        } else { AudioError::BulkConsentMismatch };
...
fn native_normalizer_rejects_mutated_schema_word_reference_and_clock() {
    ...
    assert!(matches!(INGEST_SOURCE_REGISTRY.normalize(MEETING_TRANSCRIPT_SOURCE_ID, &value.to_string()),
    ...
    let mut host = FixtureHost::small(Fault::None);
    assert!(matches!(produce_meeting_transcript(&input, &options(), &mut host),
...
fn pending_import_can_retry_the_same_artifact_without_another_inference_run() {
    ...
    let mut authorizer = Authorizer { response: ConsentResponse::Missing, requests: Vec::new() };
    assert_eq!(artifact.authorize_import(&mut authorizer).unwrap_err(), AudioError::BulkConsentRequired);
