...
fn memory_archives_keep_history_as_records_not_new_entity_types() {
    ...
    for (source, fixture) in fixtures {
        let imported = parse(source, fixture);
        assert_eq!(
            imported.normalized.records.len(),
            if source == "mem0" { 3 } else { 2 }
        );
        let texts: BTreeSet<_> = imported
            ...
            .map(|r| r.text.as_str())
            .collect();
        let expected = if source == "mem0" {
            BTreeSet::from(["earlier", "old", "new"])
        } else {
        ...
        assert!(
            imported
                .messages
                .iter()
                .all(|message| message.platform_source == source)
        );
