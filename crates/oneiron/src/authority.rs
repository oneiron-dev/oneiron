...
#[cfg(feature = "sync")]
const AUTHORITY_REPLAY_DOOR_INGEST_QUOTA_PEER_DOMAIN: &[u8] =
    b"oneiron/maintenance-ingest-quota/v1/peer";

/// Stable peer identity used by authority-log replay-door quota accounting.
#[cfg(feature = "sync")]
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct AuthorityReplayDoorPeerKey([u8; 32]);

#[cfg(feature = "sync")]
impl AuthorityReplayDoorPeerKey {
    /// Derives the quota peer key for a known authority signer.
    #[must_use]
    pub fn from_authority_key(key: &AuthorityKey) -> Self {
        match key {
            AuthorityKey::Ed25519(bytes) => Self::from_signature_key(b"ed25519", bytes),
            AuthorityKey::P256(bytes) => Self::from_signature_key(b"p256", bytes),
        }
    }

    /// Derives a fallback quota peer key before the signer is in the local roster.
    #[must_use]
    pub fn from_unknown_authority_signer(vault_id: AuthorityVaultId) -> Self {
        Self::from_signature_key(b"authority-unknown-vault", &vault_id)
    }

    /// Lowercase hex rendering for diagnostics and typed quota errors.
    #[must_use]
    pub fn hex(&self) -> String {
        crate::types::bytes_to_hex_lower(&self.0)
    }

    fn from_signature_key(suite: &[u8], public_key: &[u8]) -> Self {
        let mut hasher = blake3::Hasher::new();
        hasher.update(AUTHORITY_REPLAY_DOOR_INGEST_QUOTA_PEER_DOMAIN);
        hasher.update(&[0]);
        hasher.update(suite);
        hasher.update(&[0]);
...
/// Tunables for authority-log replay-door quota buckets.
#[cfg(feature = "sync")]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AuthorityReplayDoorIngestQuotaConfig {
    /// Accepted authority-log replay ops per peer per quota window.
...
#[cfg(feature = "sync")]
impl AuthorityReplayDoorIngestQuotaConfig {
    /// Builds authority replay-door quota tunables.
    #[must_use]
    pub const fn new(max_ops_per_peer_window: u32, quota_window_secs: u64) -> Self {
        Self {
...
/// Observable state after an accepted authority replay-door quota debit.
#[cfg(feature = "sync")]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AuthorityReplayDoorIngestQuotaSnapshot {
    ...
    /// Accepted authority-log replay ops in the bucket after this debit.
...
#[cfg(feature = "sync")]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct AuthorityReplayDoorIngestQuotaBucket {
    window_start_secs: u64,
    accepted_count: u32,
}

/// In-memory per-peer quota for authority-log replay-door admission.
#[cfg(feature = "sync")]
#[derive(Debug, Clone)]
pub struct AuthorityReplayDoorIngestQuota {
    config: AuthorityReplayDoorIngestQuotaConfig,
    buckets: BTreeMap<AuthorityReplayDoorPeerKey, AuthorityReplayDoorIngestQuotaBucket>,
}

#[cfg(feature = "sync")]
impl AuthorityReplayDoorIngestQuota {
    /// Creates empty quota state from validated tunables.
    pub fn new(config: AuthorityReplayDoorIngestQuotaConfig) -> Result<Self> {
        validate_authority_replay_door_ingest_quota_config(config)?;
        Ok(Self {
            config,
    ...
    /// Debits one authority-log replay ingest for `peer_key` at `now_secs`.
    pub fn accept_peer(
        &mut self,
        peer_key: AuthorityReplayDoorPeerKey,
        now_secs: u64,
    ) -> Result<AuthorityReplayDoorIngestQuotaSnapshot> {
        let window_start_secs = now_secs - (now_secs % self.config.quota_window_secs);
        let bucket = self
            .buckets
            .entry(peer_key)
            .or_insert(AuthorityReplayDoorIngestQuotaBucket {
                window_start_secs,
                accepted_count: 0,
            });
        if bucket.window_start_secs != window_start_secs {
            bucket.window_start_secs = window_start_secs;
            bucket.accepted_count = 0;
        }
        if bucket.accepted_count >= self.config.max_ops_per_peer_window {
            return Err(Error::MaintenanceIngestQuotaExceeded {
                peer_key_hex: peer_key.hex(),
                accepted_count: bucket.accepted_count,
                max_ops_per_peer_window: self.config.max_ops_per_peer_window,
                window_start_secs,
                quota_window_secs: self.config.quota_window_secs,
            });
        }
        bucket.accepted_count =
            bucket
                ...
                .ok_or(Error::ArithmeticOverflow(
                    "authority replay-door ingest quota",
                ))?;
        Ok(AuthorityReplayDoorIngestQuotaSnapshot {
            peer_key_hex: peer_key.hex(),
            window_start_secs,
            accepted_count: bucket.accepted_count,
            max_ops_per_peer_window: self.config.max_ops_per_peer_window,
...
/// Decodes and verifies an authority-log body, then debits the peer's replay-door quota.
#[cfg(feature = "sync")]
pub fn accept_authority_replay_door_ingest(
    bytes: &[u8],
    peer_key: AuthorityReplayDoorPeerKey,
    now_secs: u64,
    quota: &mut AuthorityReplayDoorIngestQuota,
) -> Result<AuthorityLogEntry> {
    let entry = decode_authority_log_entry_body(bytes)?;
    quota.accept_peer(peer_key, now_secs)?;
    Ok(entry)
}

#[cfg(feature = "sync")]
fn validate_authority_replay_door_ingest_quota_config(
    config: AuthorityReplayDoorIngestQuotaConfig,
) -> Result<()> {
    if config.max_ops_per_peer_window == 0 {
        return Err(Error::InvalidConfig(
            "authority replay-door ingest quota max_ops_per_peer_window must be greater than zero"
                .to_owned(),
        ));
    }
    if config.quota_window_secs == 0 {
        return Err(Error::InvalidConfig(
            "authority replay-door ingest quota quota_window_secs must be greater than zero"
                .to_owned(),
        ));
    }
    Ok(())
...
mod tests {
    ...
    ) -> AuthorityReplayDoorIngestQuota {
        AuthorityReplayDoorIngestQuota::new(AuthorityReplayDoorIngestQuotaConfig::new(
            max_ops_per_peer_window,
            quota_window_secs,
        ))
        .unwrap()
    ...
    #[cfg(feature = "sync")]
    fn replay_peer_key(entry: &AuthorityLogEntry) -> AuthorityReplayDoorPeerKey {
        AuthorityReplayDoorPeerKey::from_authority_key(entry.signer_key())
    }

    #[cfg(feature = "sync")]
    #[test]
    fn over_quota_peer_rejected() {
        let entry = genesis_entry(41, 86_400, 1);
        let bytes = encode_authority_log_entry_body(&entry).unwrap();
        let peer_key = replay_peer_key(&entry);
        let mut quota = replay_quota(1, 60);

        assert_eq!(
            accept_authority_replay_door_ingest(&bytes, peer_key, 121, &mut quota).unwrap(),
            entry
        );
        let err = accept_authority_replay_door_ingest(&bytes, peer_key, 122, &mut quota)
            .expect_err("same peer must reject after reaching the replay-door quota");
        ...
        assert!(matches!(
            ...
            } if peer_key_hex == peer_key.hex()
        ));
    }

    #[cfg(feature = "sync")]
    #[test]
    fn other_peer_unaffected() {
        let peer_a_entry = genesis_entry(42, 86_400, 1);
        let peer_b_entry = genesis_entry(43, 86_400, 1);
        let peer_a_bytes = encode_authority_log_entry_body(&peer_a_entry).unwrap();
        let peer_b_bytes = encode_authority_log_entry_body(&peer_b_entry).unwrap();
        let peer_a_key = replay_peer_key(&peer_a_entry);
        let peer_b_key = replay_peer_key(&peer_b_entry);
        let mut quota = replay_quota(1, 60);

        accept_authority_replay_door_ingest(&peer_a_bytes, peer_a_key, 180, &mut quota).unwrap();
        let err = accept_authority_replay_door_ingest(&peer_a_bytes, peer_a_key, 181, &mut quota)
            .expect_err("flooding peer must be over quota");
        assert!(matches!(
            err,
            Error::MaintenanceIngestQuotaExceeded {
                peer_key_hex,
                ...
                quota_window_secs: 60,
            } if peer_key_hex == peer_a_key.hex()
        ));

        assert_eq!(
            accept_authority_replay_door_ingest(&peer_b_bytes, peer_b_key, 182, &mut quota)
                .unwrap(),
            peer_b_entry
        );
    }
    ...
    #[cfg(feature = "sync")]
    #[test]
    fn quota_resets_per_window() {
        let entry = genesis_entry(44, 86_400, 1);
        let bytes = encode_authority_log_entry_body(&entry).unwrap();
        let peer_key = replay_peer_key(&entry);
        let mut quota = replay_quota(1, 10);

        accept_authority_replay_door_ingest(&bytes, peer_key, 19, &mut quota).unwrap();
        let same_window_err = accept_authority_replay_door_ingest(&bytes, peer_key, 19, &mut quota)
        ...
        accept_authority_replay_door_ingest(&bytes, peer_key, 20, &mut quota).unwrap();
        let next_window_err = accept_authority_replay_door_ingest(&bytes, peer_key, 20, &mut quota)
