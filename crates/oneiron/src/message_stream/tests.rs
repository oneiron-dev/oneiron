...
fn mode_precedence_and_bound_chat_override_are_effective() {
    ...
    vault.set_buffered_message_stream_policy(&policy, actor).unwrap();
    let default = vault
...
fn idle_sweep_returns_successes_and_refusals_without_losing_receipts() {
    for refused_first in [false, true] {
        let (_dir, vault, actor) = fixture();
        let good_id = crate::test_util::entity(if refused_first { 2 } else { 1 });
        let bad_id = crate::test_util::entity(if refused_first { 1 } else { 2 });
        let good = vault
        ...
        for state in vault.buffered_message_streams.lock().unwrap().values_mut() {
