...
impl ScopedRead<'_> {
        ...
        let value = if self.is_entity_retrievable_with_policy_in(&txn, &policy, &filter, id)? {
            ...
            for edge in self.edges_out_in(&txn, id)? {
                if self.is_entity_retrievable_with_policy_in(
                    ...
                    kept.push(edge);
                } else if self.entities().get(&txn, edge.target.as_bytes())?.is_some() {
                    suppressed += 1;
                }
            }
            Some(kept)
        } else {
            suppressed += usize::from(self.entities().get(&txn, id.as_bytes())?.is_some());
            None
        };
        Ok(ScopedReadResult {
            value,
    ...
    pub fn memory_timeline(&self, anchor: &EntityId) -> Result<ScopedReadResult<MemoryTimeline>> {
        ...
        for record in timeline.records {
            ...
            {
                kept.push(record);
            } else if self.entities().get(&txn, record.id.as_bytes())?.is_some() {
                suppressed += 1;
            }
        }
    ...
    fn timeline_record_allowed_in(
        ...
        let Some(raw) = self.entities().get(txn, record.id.as_bytes())? else {
            return Ok(false);
        };
        let header =
            EntityMetadataHeader::parse(&raw).ok_or(Error::CorruptedIndex("entity header"))?;
