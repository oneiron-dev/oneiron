...
use super::{AsrEvent, AsrEventKind, AsrStream, AsrToken};
...
impl<T: SonioxTransport> SonioxAsrClient<T> {
    ...
    pub fn receive(&mut self, frame: &serde_json::Value) -> Result<Vec<AsrEvent>> {
        if frame.get("error_code").is_some() {
            return Ok(vec![AsrEvent {
                kind: AsrEventKind::Error,
                text: String::new(),
                tokens: vec![],
                provider_latency_ms: None,
                endpoint_delay_ms: None,
                error: Some(frame["error_code"].to_string()),
            }]);
        }
        ...
        if let Some(tokens) = frame.get("tokens") {
            for token in tokens
                .as_array()
                .ok_or_else(|| invalid("Soniox tokens must be an array"))?
            {
                let text = token["text"]
                    .as_str()
                    .ok_or_else(|| invalid("Soniox token text missing"))?;
                if text == "<end>" {
                    endpoint = true;
                    continue;
                }
                let normalized = AsrToken {
                    text: text.into(),
                    is_final: token["is_final"]
                        .as_bool()
                        .ok_or_else(|| invalid("Soniox final flag missing"))?,
                    start_ms: optional_number(token, "start_ms")?,
                    end_ms: optional_number(token, "end_ms")?,
                    confidence: optional_number(token, "confidence")?,
                };
                let validation = event(AsrEventKind::Partial, vec![normalized.clone()]);
                validation.validate()?;
                if normalized.is_final {
                    self.committed.push(normalized);
                } else {
                    partial.push(normalized);
                }
            }
        }
        ...
        if endpoint {
            ...
            let mut hypothesis = self.committed.clone();
            hypothesis.extend(partial);
            events.push(event(AsrEventKind::Partial, hypothesis));
        }
        if frame.get("finished").and_then(serde_json::Value::as_bool) == Some(true) {
            ...
            events.push(event(AsrEventKind::Closed, Vec::new()));
            self.ended = true;
        }
        for event in &events {
            event.validate()?;
...
fn optional_number(value: &serde_json::Value, key: &str) -> Result<Option<f64>> {
    match value.get(key) {
        ...
        Some(value) => value
            .as_f64()
            .map(Some)
            .ok_or_else(|| invalid("Soniox numeric metadata invalid")),
    }
}
...
impl<T: SonioxTransport> AsrStream for SonioxAsrClient<T> {
    fn accept_audio(&mut self, pcm: &[u8]) -> Result<()> {
        if self.ended || pcm.is_empty() {
            return Err(invalid("Soniox stream closed or audio empty"));
        }
        self.transport.submit_audio(pcm)
    }
    fn end(&mut self) -> Result<()> {
        if !self.ended {
            self.transport.submit_end()?;
            self.ended = true;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn audio_submission_and_partial_final_endpoint_normalize() -> Result<()> {
        let mut client = SonioxAsrClient::new(Transport::default());
        client.accept_audio(&[1, 2, 3, 4])?;
        ...
        assert_eq!(partial[0].kind, AsrEventKind::Partial);
        partial[0].validate()?;
        let final_events = client.receive(&serde_json::json!({"tokens":[{"text":"hello","is_final":true},{"text":"<end>","is_final":true}]}))?;
        assert_eq!(final_events[0].kind, AsrEventKind::Final);
        assert_eq!(final_events[0].text, "hello");
        assert_eq!(final_events[1].kind, AsrEventKind::Endpoint);
        ...
        assert!(client.accept_audio(&[0]).is_err());
        Ok(())
    }
}
