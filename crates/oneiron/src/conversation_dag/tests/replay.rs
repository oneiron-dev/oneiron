...
fn sub_session_membership_survives_entity_and_edge_replay() {
    ...
    let asking = source
        ...
        .unwrap()
        .id;
    let session = source.spawn_sub_session(&asking, actor).unwrap();
    let mut worker = input(conv, Some(asking), false, actor);
    worker.session = Some(session);
    let worker = source.append_record(&worker).unwrap().id;
    let trunk = source
    ...
    assert_eq!(
        peer.resolve_scope(&scope(conv, ScopePath::SubSession(session), false))
            .unwrap()
            .records,
        [worker]
    );
    assert_eq!(
        peer.resolve_scope(&scope(conv, ScopePath::Canonical, true))
            .unwrap()
            .records,
        [asking, trunk]
    );
    assert!(
        peer.resolve_scope(&scope(conv, ScopePath::Branch(worker), false))
            .is_err()
    );
...
fn malformed_and_conflicting_session_carriers_refuse_without_changing_the_record() {
    ...
    let worker = vault.append_record(&append).unwrap().id;
