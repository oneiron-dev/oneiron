...
/// Hex of the seeded root project row, the one structural row every fresh
/// vault carries. Whole-vault export flags it as an owning-adapter refusal.
fn source_root_id(vault: &Vault) -> Result<String> {
    let rtxn = vault.store.env.read_txn()?;
    let raw = vault
        .store
        .vault_meta
        .get(&rtxn, b"project.root.v1")?
        .expect("seeded root project");
    Ok(crate::EntityId::from_bytes(raw.as_ref().try_into().expect("root id"))?.to_hex())
}
...
fn foreign_archive_authority_and_witness_rows_remain_data_not_local_rights() -> Result<()> {
    ...
    assert_eq!(
        ...
        vec![crate::batch::export::ExportImportRefusal::Entity {
            entity_id: source_root_id(&source)?,
