...
use super::{
    AgentDispatchOutcome, AgentDispatchTarget, AgentDispatcher, AgentSpawnContext, DispatchAgent,
};
...
impl AgentDispatcher<'_> {
    pub(super) fn dispatch_workflow(
        ...
        let definition = self
            .vault
            .get_workflow(&id)?
            .ok_or_else(|| invalid("saved workflow is missing"))?;
        let spawn = AgentSpawnContext {
            context_spec: spawn.context_spec.map(normalize_context_spec),
            ..spawn
        };
        // A parented composition may not inherit the non-dispatch compatibility
        // fallback. Its wrapper must retain provable live agent lineage.
        ...
        // Validate every later step before a wrapper, fork or attempt is durable.
        // Widening is an explicit refusal here, never a silent dropped bound or
        // a proposal which would accidentally launch only one leaf on approval.
        for step in &definition.steps {
            let live = self.dispatchable_definition(&AgentDispatchTarget::Custom(*step))?;
        ...
        match crate::vault::live_entity_row_in_txn(&self.vault.store, &txn, &id)? {
            crate::vault::LiveEntityRow::Live { entity_type, body }
                if entity_type == crate::registry::ENTITY_TYPE_WORKFLOW
                    && body == intent.definition => {}
            _ => return Err(invalid("workflow changed during admission")),
        }
        if let Some(parent) = input.parent_attempt {
            self.parent_dispatch_input_in_txn(&txn, parent)?
                .ok_or_else(|| invalid("workflow parent disappeared during admission"))?;
        }
        ...
        record.active = self.enqueue_workflow_step(&mut txn, &record, 0, input.now)?;
        self.vault
            .store
            .vault_meta
            .put(&mut txn, &record_key(root.id), &json(&record)?)?;
        if let Some(key) = input.dedupe_key.as_deref() {
            self.vault
                .store
                .vault_meta
                .put(&mut txn, &dedupe_key(key), root.id.as_bytes())?;
        }
