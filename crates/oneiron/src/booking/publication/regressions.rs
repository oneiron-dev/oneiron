use super::*;
use crate::booking::{
    ActiveHoldSource, HoldLeaseSpec, HoldSpec, SessionKey, VaultActiveHoldSource,
};
...
mod tests {
    ...
    fn publication_copied_owner_evidence_cannot_use_raw_batch_or_replay_doors() {
        ...
        assert!(
            vault
                .batch()
                .put(&id(8), crate::registry::ENTITY_TYPE_CLAIM, time, 1, &bytes)
                .commit()
                .is_err()
        );
        assert!(
            vault
                .batch()
                .put_replicated(&id(8), crate::registry::ENTITY_TYPE_CLAIM, time, 1, &bytes)
                .commit()
                .is_err()
        );
        ...
        let envelope = WriteEnvelope::new(
            WriteActor::new(id(2), EdgeActorClass::Human),
            ClaimSource::UserStated,
            WriteProvenance::new(rmpv::Value::from("copied owner")).expect("provenance"),
            ClaimApprovalStatus::Auto,
        );
        assert!(
            vault
                .batch()
                .claim_candidate(&id(8), candidate, &envelope, time, 1)
                .commit()
                .is_err()
        );
        assert_eq!(
...
fn publication_token_lookup_is_durable_and_misses_do_not_scan_claims() {
    ...
    assert_eq!(
        resolve_public_booking_token(&vault, &token).expect("private address"),
        None
    );
    vault
        .memory(id(2), EdgeActorClass::Human)
    ...
    for byte in 10..30 {
        ...
        assert_eq!(
            resolve_public_booking_token(&vault, &unknown).expect("fixed-key miss"),
            None
        );
    }
    assert_eq!(
        resolve_public_booking_token(&vault, &token).expect("hit"),
        Some(id(1))
    );
    drop(vault);
    let vault = Vault::open(dir.path(), crate::VaultConfig::default()).expect("reopen");
    assert_eq!(
        resolve_public_booking_token(&vault, &token).expect("durable"),
        Some(id(1))
    );
}
...
fn publication_exact_config_hash_rejects_same_duration_host_or_buffer_substitution() {
    ...
    vault
        .put_claim(&id(3), &replacement, TimeRange { start: 1, end: 1 }, 1)
...
fn public_hold_writer_rechecks_retraction_expiry_and_exact_configuration() {
    for change in ["retract", "expire", "config", "unpublish"] {
        ...
        match change {
            ...
            "unpublish" => {
                claim_input.value["published"] = json!(false);
                owner.claim_upsert(&claim_input).expect("withdraw");
            }
            _ => {
                let mut config_body = vault.get_claim(&id(5)).expect("config").expect("body");
                let mut value = crate::booking::decode_event_type_claim_value(&config_body.value)
                    .expect("value");
                value.config.post_buffer_min += 1;
                config_body.value = encode_event_type_claim_value(&value).expect("changed config");
                vault
                    .put_claim(&id(5), &config_body, TimeRange { start: 1, end: 1 }, 1)
                    .expect("config write");
            }
        }
        let spec = HoldSpec {
            ...
            session_key: SessionKey::derive(b"public transaction"),
        ...
        assert!(
            crate::booking::lifecycle::execute_hold(&vault, &spec, write_now, Some(&authority))
                .is_err(),
            "{change}"
        );
        assert!(
            VaultActiveHoldSource::new(&vault)
                .active_holds(id(1), spec.slot, now, None)
                .expect("holds")
                .is_empty()
        );
    }
...
fn public_attempt_dedupe_cannot_alias_a_literal_non_public_key() {
    ...
    let spec = HoldSpec {
        ...
        session_key: SessionKey::derive(b"public dedupe"),
        visitor_tz: "UTC".to_owned(),
        constraint: None,
        lease: HoldLeaseSpec::Ordinary,
    ...
    let private_id =
        enqueue_booking_verb(&vault, BookingVerbRequest::Hold(private_spec.clone()), now)
    ...
    let private_first =
        enqueue_booking_verb(&other_vault, BookingVerbRequest::Hold(private_spec), now)
    ...
    assert_eq!(
        enqueue_booking_verb_with_publication(
            &vault,
    ...
    let public_attempt: BookingLifecycleAttempt =
        rmp_serde::from_slice(&public_row.payload[1..]).expect("public payload");
    assert_eq!(public_attempt.public_authority, Some(authority));
    let private_row = queue
        .get(private_id)
        .expect("read")
        .expect("non-public row");
    let private_attempt: BookingLifecycleAttempt =
        rmp_serde::from_slice(&private_row.payload[1..]).expect("non-public payload");
...
#[test]
fn publication_put_rejects_corrupt_existing_claim_but_allows_erased_shell() -> Result<()> {
    let (_dir, vault) = open();
    let claim = id(5);
    let body = vault.get_claim(&claim)?.expect("configuration");
    let bytes = crate::claim::encode_claim_body(&body)?;
    vault.delete_entity_with_reason(&claim, crate::DeleteReason::UserDelete)?;
    let shell = vault.get_raw(&claim)?.expect("erased shell");
    assert_eq!(shell.len(), crate::batch::ENTITY_METADATA_HEADER_LEN);
    {
        let txn = vault.store.env.read_txn()?;
        guard_publication_put(&vault.store, &txn, claim, Some(&body))?;
    }

    let mut trailing = bytes.clone();
    trailing.push(0xc0);
    let mut corrupt_rows = vec![shell[..shell.len() - 1].to_vec()];
    for corrupt_body in [vec![0xc1], vec![0xc0], vec![0x80], trailing] {
        let mut raw = shell.clone();
        raw.extend_from_slice(&corrupt_body);
    ...
    for corrupt in corrupt_rows {
        ...
        {
            let txn = vault.store.env.read_txn()?;
            // Both CLAIM and non-CLAIM overwrites must inspect the old row.
            for incoming in [Some(&body), None] {
                let error = guard_publication_put(&vault.store, &txn, claim, incoming)
                    .expect_err("corrupt existing row is not an erased shell");
                let expected = if corrupt.len() < crate::batch::ENTITY_METADATA_HEADER_LEN {
                    crate::error::ErrorKind::CorruptedIndex
                } else {
                    crate::error::ErrorKind::InvalidClaimBody
                };
    ...
    assert_eq!(vault.get_claim(&claim)?, Some(body));
    Ok(())
}

#[test]
fn publication_put_rejects_malformed_incoming_claim_on_erased_shell() -> Result<()> {
    let (_dir, vault) = open();
    let claim = id(5);
    let body = vault.get_claim(&claim)?.expect("configuration");
    let mut trailing = crate::claim::encode_claim_body(&body)?;
    trailing.push(0xc0);
    vault.delete_entity_with_reason(&claim, crate::DeleteReason::UserDelete)?;
    let shell = vault.get_raw(&claim)?.expect("erased shell");
    assert_eq!(shell.len(), crate::batch::ENTITY_METADATA_HEADER_LEN);
    for malformed in [Vec::new(), vec![0xc1], vec![0xc0], vec![0x80], trailing] {
        let error = vault
            .batch()
            .put_replicated(
                &claim,
            ...
            .commit()
            .expect_err("erased predecessor does not exempt incoming validation");
        assert_eq!(error.kind(), crate::error::ErrorKind::InvalidClaimBody);
        assert_eq!(vault.get_raw(&claim)?, Some(shell.clone()));
    }
    Ok(())
}

fn assert_publication_slot_rejects_raw_overwrites(
    vault: &Vault,
    claim: EntityId,
    publication_body: &ClaimBody,
    ordinary_body: &ClaimBody,
) -> Result<()> {
    let before = vault.get_raw(&claim)?;
    let time = TimeRange { start: 1, end: 1 };
    for body in [publication_body, ordinary_body] {
        assert!(vault.put_claim(&claim, body, time, 1).is_err());
        let bytes = crate::claim::encode_claim_body(body)?;
        for replicated in [false, true] {
            let batch = vault.batch();
            let batch = if replicated {
                batch.put_replicated(&claim, crate::registry::ENTITY_TYPE_CLAIM, time, 1, &bytes)
            } else {
                batch.put(&claim, crate::registry::ENTITY_TYPE_CLAIM, time, 1, &bytes)
            };
    ...
    for replicated in [false, true] {
        ...
        let error = batch
            .commit()
            .expect_err("non-CLAIM overwrite is owner-only too");
        assert!(matches!(
            error,
            Error::InvalidClaimBody("booking publication requires the owner memory write door")
        ));
        assert_eq!(vault.get_raw(&claim)?, before);
    }
    let txn = vault.store.env.read_txn()?;
...
#[test]
fn publication_slots_stay_owner_only_after_head_change_soft_delete_and_reopen() -> Result<()> {
    let (dir, vault) = open();
    let ordinary_body = vault.get_claim(&id(5))?.expect("configuration");
    let owner = vault.memory(id(2), EdgeActorClass::Human);
    let first = owner.claim_upsert(&input(publication())).expect("publish");
    let first_id =
        crate::memory::resolve_entity_ref(&vault, &first.claim_short_id).expect("first claim id");
    let mut updated = publication();
    updated.owner_display = "Updated owner".to_owned();
    let second = owner.claim_upsert(&input(updated.clone())).expect("update");
    let second_id =
        crate::memory::resolve_entity_ref(&vault, &second.claim_short_id).expect("second claim id");
    assert_ne!(first_id, second_id);
    assert_eq!(
        vault.pending_claim_supersession(&second_id)?,
        Some(first_id)
    );
    assert!(load_public_booking_page(&vault, id(1), 150)?.is_none());
    assert!(
        vault
    ...
    let bodies = [
        vault.get_claim(&first_id)?.expect("superseded publication"),
        vault.get_claim(&second_id)?.expect("current publication"),
    ];
    for (claim, body) in [first_id, second_id].into_iter().zip(&bodies) {
        assert_publication_slot_rejects_raw_overwrites(&vault, claim, body, &ordinary_body)?;
        vault.delete_entity_with_reason(&claim, crate::DeleteReason::UserDelete)?;
        assert_eq!(
            vault.get_raw(&claim)?.expect("publication shell").len(),
    ...
    // An unrelated erased shell is not permission to create a publication by
    // copying the owner's evidence, even though ordinary reannotation is legal.
    vault.delete_entity_with_reason(&id(5), crate::DeleteReason::UserDelete)?;
    let shell = vault.get_raw(&id(5))?.expect("configuration shell");
    let bytes = crate::claim::encode_claim_body(&bodies[1])?;
    assert!(
        vault
            .put_claim(&id(5), &bodies[1], TimeRange { start: 1, end: 1 }, 1)
            .is_err()
    );
    assert!(
        vault
    ...
    assert_eq!(vault.get_raw(&id(5))?, Some(shell));
    Ok(())
}
