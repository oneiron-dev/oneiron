...
fn a_pending_epoch_summary_offers_its_text_to_the_embedder() -> Result<()> {
    ...
    let body = crate::compaction::encode_epoch_summary_body(&crate::compaction::EpochSummaryBody {
    ...
    vault.with_write_txn(|wtxn| {
        ...
        assert_eq!(
            input.claim_body,
            b"the epoch prose".to_vec(),
            "the embedder receives the summary text"
        );
        assert_ne!(
            input.claim_body, body,
            "the MessagePack record is not what gets embedded"
        );
        Ok(())
    })?;
    Ok(())
