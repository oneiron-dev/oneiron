...
pub(super) fn validate(snapshot: &CanonicalSnapshot) -> Result<()> {
    ...
    for edge in &snapshot.base_edges {
        id(edge.source)?;
        id(edge.target)?;
        if deleted.get(&edge.source).is_some_and(|row| row.is_hard())
            || deleted.get(&edge.target).is_some_and(|row| row.is_hard())
        {
...
pub(super) fn validate_documents(snapshot: &CanonicalSnapshot) -> Result<()> {
    ...
    for row in &snapshot.doc_snapshots {
        ...
        if core.document_head.is_none() || !heads.contains_key(&row.entity_id) {
            return Err(invalid("document has no core/head binding"));
        }
    }
    for (entity, core) in &cores {
        if let Some(birth) = core.document_head {
            if !docs.contains(&(*entity, *birth.as_bytes())) || !heads.contains_key(entity) {
                return Err(invalid("document-backed core is missing its birth/head"));
            }
        } else if heads.contains_key(entity) {
            return Err(invalid("inline NOTE has a document head"));
        }
    }
    ...
    for row in &snapshot.head_move_receipts {
        ...
        {
            return Err(invalid("receipt head absent"));
        }
        match receipt.verdict {
            crate::note::NoteVerdict::Switch => {
                if receipt.head != receipt.fork
                    || !docs.contains(&(row.entity_id, *receipt.fork.as_bytes()))
                {
                    return Err(invalid("switch receipt target"));
                }
            }
            crate::note::NoteVerdict::Merge | crate::note::NoteVerdict::Reject => {
                if receipt.head != receipt.previous_head {
                    return Err(invalid("non-switch moved head"));
                }
            }
        }
    }
    for row in &snapshot.document_heads {
        id(row.entity_id)?;
        id(row.head)?;
        let core = cores.get(&row.entity_id).ok_or(invalid("head owner"))?;
        if !docs.contains(&(row.entity_id, row.head)) {
            return Err(invalid("head document absent"));
        }
        if core.document_head.map(|head| *head.as_bytes()) != Some(row.head) {
