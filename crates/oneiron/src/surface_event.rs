...
fn route_inbound_surface_event(
    ...
    // Resolved once, before the state fan-out, so every routed arm stamps the
    // same subject. An unanchored actor yields `None` and still routes.
    let subject_ref = crate::subject_model::actor_subject_anchor(vault, &actor_ref)?;
