...
fn retained_subsession_scope_covers_300_body_truth_edges_cap_and_one_header() {
    ...
    for n in 0..300 {
        ...
        parent = vault.append_record(&next).unwrap().id;
        turns.push(parent);
    }
    let selector = scope(conv, ScopePath::SubSession(session), false);
    assert_eq!(vault.resolve_scope(&selector).unwrap().records, turns);
    assert_eq!(
        vault
    ...
    let summary = vault
        .mint_scope_summary(&selector, "  handed text verbatim  ", actor)
        .unwrap();
...
fn subsessions_are_isolated_and_second_spawned_by_is_refused_or_detected() {
    ...
    let asking = vault
        ...
        .id;
    let other = vault
        .append_record(&input(conv, Some(asking), true, actor))
        .unwrap()
        .id;
    let session = vault.spawn_sub_session(&asking, actor).unwrap();
    let sibling = vault.spawn_sub_session(&other, actor).unwrap();
    let mut next = input(conv, Some(asking), false, actor);
    next.session = Some(session);
    let child = vault.append_record(&next).unwrap().id;
    next.parent = Some(child);
    next.session = Some(sibling);
    assert_eq!(
        vault.append_record(&next).unwrap_err().kind(),
    ...
    assert_eq!(
        vault.resolve_scope(&contradictory).unwrap_err().kind(),
        ErrorKind::InvalidConversationDag
    );
    assert_eq!(
        vault
...
fn denied_header_and_reply_roll_back_then_granted_late_result_advances_trunk() {
    ...
    let mut next = input(conv, Some(asking), false, actor);
    next.session = Some(session);
    let worker = vault.append_record(&next).unwrap().id;
    let current = vault
        .append_record(&input(conv, Some(asking), true, actor))
        .unwrap()
        .id;
    let selector = scope(conv, ScopePath::SubSession(session), false);
    let summary = vault
        .mint_scope_summary(&selector, "The result", actor)
        .unwrap();
    ...
    assert_eq!(vault.resolve_scope(&selector).unwrap().records, [worker]);
}
...
fn nested_subsession_continues_its_spawning_turn_without_leaking_scopes() {
    ...
    let asking = vault
        ...
        .unwrap()
        .id;
    let outer = vault.spawn_sub_session(&asking, actor).unwrap();
    let mut next = input(conv, Some(asking), false, actor);
    next.session = Some(outer);
    let outer_turn = vault.append_record(&next).unwrap().id;
    let inner = vault.spawn_sub_session(&outer_turn, actor).unwrap();
    next.parent = Some(outer_turn);
    next.session = Some(inner);
    let inner_turn = vault.append_record(&next).unwrap().id;
    assert_eq!(
        vault
            .resolve_scope(&scope(conv, ScopePath::SubSession(outer), true))
            .unwrap()
            .records,
        [outer_turn]
    );
    assert_eq!(
        vault
            .resolve_scope(&scope(conv, ScopePath::SubSession(inner), true))
            .unwrap()
            .records,
        [inner_turn]
    );
