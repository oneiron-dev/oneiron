...
fn owner_dial_is_space_local_receipted_and_never_expands_room_or_grants_send() -> crate::Result<()>
{
    ...
    let room = ResolvedContextProjection {
        memory_sections: vec!["room:claim".to_owned()],
        ..Default::default()
    };
    ...
    let receipt = vault.set_space_posting_preset(
        identity,
        "group:one",
    ...
    let bound = planned
        .needs_owner_consent
        .expect("dial cannot mint permission");
    assert_eq!(
        vault
            .space_posting_plan(identity, "group:two", room.clone())?
            .preset,
    ...
    assert!(
        events
            .iter()
            .any(|event| event.decision_id == receipt.decision_id
                && event.actor_ref.as_deref() == Some(owner.actor().to_hex().as_str()))
    );
    Ok(())
}
...
fn bot_only_platform_refuses_owner_presentation_and_keeps_named_default() -> crate::Result<()> {
    ...
    assert_eq!(
        vault
            .space_posting_plan(id, "group:one", Default::default())?
            .preset,
        GroupPostingPreset::NamedParticipant
    );
    assert!(
        crate::outbound::outbound_verb_contract("email", "send")
            .unwrap()
            .params["posting"]["policy_risk"]
            .as_bool()
            .unwrap()
    );
    Ok(())
