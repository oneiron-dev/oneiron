...
fn temp_vault() -> (tempfile::TempDir, Vault) {
    // The fixture must not inherit a symlinked system temp root (macOS /var).
    // Production deliberately refuses symlink ancestors for secret files.
    ...
    assert!(
        matches!(
            ...
            Error::Secret(SecretError::SecretLeasePathRefused { path: refused, .. })
                if refused == &parent.display().to_string()
        ),
        "got {err:?}"
    );
    assert!(
