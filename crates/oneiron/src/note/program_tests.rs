...
fn current(vault: &Vault, id: EntityId) -> String {
    vault.note_program_document(id).unwrap().unwrap().text()
}
...
fn source_bridge_is_lazy_and_source_is_unchanged() {
    ...
    assert!(vault.note_program_document(id).unwrap().is_none());
    assert_eq!(
...
fn agent_facade_and_pack_project_live_document_text() {
    ...
    for kind in [
        ...
        let doc = vault.note_program_document(id).unwrap().unwrap();
        memory
            .edit_note(
                &receipt.id_hex,
                &NoteEdit::InsertAfter {
...
fn recovered_merge_refuses_ambiguous_overlap_without_losing_bundle_members() {
    ...
    vault
        ...
        .unwrap();
    let read = vault.note_program_document(second).unwrap().unwrap();
    vault
        .edit_note(
            second,
            &NoteEdit::InsertAfter {
