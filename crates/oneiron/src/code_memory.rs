...
use crate::ppr::{
    self, SeedWeighting, flush_deferred_ppr_cache_writes, ppr_query_in_txn_with_deferred_cache,
};
...
/// 2. ONE `RoTxn`: seeds must be live `CODE_SYMBOL`s, then the landed raw-seed
///    PPR entry at [`CODE_MEMORY_PPR_DEPTH`], alpha `0.15`,
///    [`SeedWeighting::Specificity`] (`lambda_for_kind(Blocks) == None` keeps
///    readiness edges out of the walk);
...
///    are forbidden), and clamp every referenced entity;
