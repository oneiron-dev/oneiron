...
#[test]
fn open_rejects_an_abi_18_vault() -> Result<()> {
    let temp_dir = tempfile::tempdir()?;
    let path = temp_dir.path();
    {
        let _vault = Vault::open(path, test_config())?;
    }
    set_raw_storage_abi_version(path, Some(18))?;

    let err = match Vault::open(path, test_config()) {
        Ok(_) => panic!("expected Vault::open to reject an ABI 18 vault"),
        Err(err) => err,
    };
    assert!(
        matches!(
...
#[test]
fn open_rejects_an_abi_20_vault() -> Result<()> {
    let temp_dir = tempfile::tempdir()?;
    let path = temp_dir.path();
    {
        let _vault = Vault::open(path, test_config())?;
    }
    set_raw_storage_abi_version(path, Some(20))?;

    let err = match Vault::open(path, test_config()) {
        Ok(_) => panic!("expected Vault::open to reject an ABI 20 vault"),
        Err(err) => err,
    };
    assert!(
        matches!(
