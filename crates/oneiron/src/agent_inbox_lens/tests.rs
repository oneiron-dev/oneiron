...
fn approval_pile_filters_identity() {
    ...
    let b = entity(0x62);
    for (task, receipt) in [
        (entity(0x31), held(a, "a", 5, 1)),
        (entity(0x32), held(b, "b", 100, 2)),
    ] {
    ...
    assert!(
        vault
            .agent_inbox_lens(query(Some(entity(0x63)), 10))
            .unwrap()
            .is_empty()
    );
}
...
fn approval_order_and_cursor_are_stable() {
    ...
    assert_eq!(
        all.iter()
            .map(|i| i.receipt_ref.as_deref().unwrap())
            .collect::<Vec<_>>(),
    ...
    reversed.reverse();
    assert_eq!(
        project_approval_required(&reversed, query(None, 10)).unwrap(),
        all
    );
    let first = project_approval_required(&receipts, query(None, 2)).unwrap();
    let cursor = first.last().unwrap();
    let rest = project_approval_required(
        &receipts,
        AgentInboxLensQuery {
            identity_ref: None,
            limit: 10,
            before: Some((cursor.impact, cursor.occurred_at, cursor.item_id.clone())),
        },
    )
    .unwrap();
...
fn completed_sends_and_non_approval_holds_leave_the_pile() {
    ...
    assert!(
        project_approval_required(&[window_hold, draft], query(None, 10))
            .unwrap()
            .is_empty()
    );
}
...
fn below_cap_filtered_source_stays_incomplete_and_inbox_fails_closed() {
    ...
    let empty = vault
        .scan_receipts(receipt_query.with_actor(entity(0x72).to_hex()))
        .unwrap();
...
fn exact_cap_source_is_complete_and_inbox_keeps_impact_top_n() {
    ...
    assert!(
        scan.complete,
        "the extra source read is empty, not overflow"
    );
    assert!(scan.continuation.is_none());
    let items = vault
        .agent_inbox_lens(query(Some(entity(0x61)), 1))
        .unwrap();
    assert_eq!(items.len(), 1);
...
fn capped_overflow_has_source_continuation_and_no_ranked_subset() {
    ...
    let scan = vault
        .scan_receipts(ReceiptQuery::new(usize::MAX).with_kind(ReceiptKind::Outbound))
        .unwrap();
    assert_eq!(scan.records.len(), MAX_RECEIPT_QUERY_SCAN);
    assert!(!scan.complete);
    ...
    assert_eq!(
        continuation.next_record.unwrap().receipt_id,
...
fn result_limit_reports_continuation_without_weakening_inbox_top_n() {
    ...
    assert_eq!(
        zero.continuation.unwrap().next_record.unwrap().receipt_id,
        "receipt:low:b"
    );
    let scan = vault
        .scan_receipts(ReceiptQuery::new(2).with_kind(ReceiptKind::Outbound))
        .unwrap();
    ...
    assert_eq!(
        continuation.next_record,
        Some(ReceiptScanPosition {
            occurred_at: 1,
            receipt_kind: ReceiptKind::Outbound,
    ...
    let exact = vault
        .scan_receipts(ReceiptQuery::new(3).with_kind(ReceiptKind::Outbound))
        .unwrap();
...
fn unproven_projectors_and_lineage_are_rejected_by_completeness_scan() {
    ...
    for receipt_query in [
        ...
        ReceiptQuery::new(1)
            .with_kind(ReceiptKind::Outbound)
            .with_job_ref("brief:1"),
    ] {
        assert!(matches!(
            vault.scan_receipts(receipt_query),
