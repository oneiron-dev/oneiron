...
/// The ABI handshake stays strictly symmetric for EVERY stored version.
#[test]
...
/// The existing-only door has no stamp-on-new branch: that is a write against a
/// vault whose identity has not been compared yet.
#[cfg(target_os = "linux")]
#[test]
fn open_existing_refuses_a_vault_stamped_at_another_storage_abi() -> Result<()> {
    ...
    drop(Vault::open(&root, config.clone())?);
    {
        ...
        vault.store.vault_meta.put(
            &mut wtxn,
            STORAGE_ABI_VERSION_KEY,
            &(STORAGE_ABI_VERSION - 1).to_le_bytes(),
        )?;
        wtxn.commit()?;
    }
    ...
    assert_eq!(*stored, Some(STORAGE_ABI_VERSION - 1));
