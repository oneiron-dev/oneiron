...
pub struct EditProposal {
    ...
    /// was run against a blob artifact (through [`super::propose_document_edit`]).
    ...
    pub engine: crate::calc::EngineId,
    pub prepared: crate::PreparedEdit,
}
...
pub fn run_edit_roundtrip<S: EditSession<E>, E: From<Error>>(
    session: &S,
    input_bytes: &[u8],
    format: OfficeFormat,
    plan: &EditPlan,
    run_ref: &str,
) -> std::result::Result<EditOutcome, E> {
    if run_ref.trim().is_empty() {
        return Err(Error::EditFailed("run_ref must be non-empty").into());
    }
    ...
    if mutation_mode == MutationMode::Minimal && plan.ops.iter().any(EditOp::is_structural) {
        return Err(Error::InvalidManifest("minimal-mutation mode refuses structural ops: preserved pivot/chart/macro parts would go stale against the shifted grid").into());
    }

    // Stage 2: targeted edit through the seam.
    let applied = session.apply_edits(&doc_before, plan)?;
    validate_ops(&applied.applied_ops)?;
    ...
    let recalc = if plan.needs_recalc(&applied.applied_ops) {
        if !session.supports_recalc() {
            return Err(Error::EditFailed("edit may change formula values but the session cannot recalc; route to a recalc-capable session").into());
        }
