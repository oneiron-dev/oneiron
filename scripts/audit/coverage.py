...
def coverage_tmp(base=None):
    if base is not None:
        base = pathlib.Path(base).resolve()
        base.mkdir(parents=True, exist_ok=True, mode=0o700)
    with tempfile.TemporaryDirectory(prefix="oc-", dir=base) as directory:
        temporary = pathlib.Path(directory).resolve()
        if sys.platform == "darwin" and len(os.fsencode(temporary)) > 70:
            raise ValueError("coverage temporary path is too long for Unix sockets; use --tmp-dir with a short real path")
        yield temporary

def collect(output, toolchain=None, unstable_doctests=False, tmp_dir=None):
    env = os.environ.copy()
    ...
    with coverage_tmp(tmp_dir) as temporary:
...
def main():
    ...
    p.add_argument("--tmp-dir", type=pathlib.Path, help="short parent for a private temporary directory; independent of the report path")
    ...
    args = p.parse_args()
    reports = collect(args.output.parent, args.toolchain, args.unstable_doctests, args.tmp_dir) if args.collect else args.reports
