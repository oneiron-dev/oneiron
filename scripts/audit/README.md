...
Pins are in `tools.json`. The CI compiler lanes use sccache 0.10.0, a persistent
...
crates, then checks outcomes. `--report path/to/outcomes.json` checks a saved
...
`python3 scripts/audit/coverage.py --collect` collects disjoint full-tier nextest,
featureless libtest and doctest lanes with pinned cargo-llvm-cov. The doctest lane
requires a toolchain with LLVM doctest instrumentation support. `coverage.py
