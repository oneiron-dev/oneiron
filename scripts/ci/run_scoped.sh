...
# Cargo compilation/test steps use --locked: a PR must commit its dependency resolution.
...
test)
  if [ "$full" = true ]; then
    ...
    run cargo test --locked --doc --workspace --exclude oneiron-bench --all-features
    exit 0
  fi
  [ ${#others[@]} -gt 0 ] && run cargo nextest run --locked "${others[@]}" --all-features --no-fail-fast --no-tests=pass
  if [ "$oneiron" = true ]; then
    if [ -n "$filter" ]; then
      run cargo nextest run --locked -p oneiron --lib --all-features --no-fail-fast --no-tests=pass -E "$filter"
    else
      run cargo nextest run --locked -p oneiron --lib --all-features --no-fail-fast
    fi
    [ "$it" = true ] && run cargo nextest run --locked -p oneiron --all-features --no-fail-fast --no-tests=pass -E 'kind(test)'
  fi
