...
ROWS = MANIFEST['verbs']
NAMES = [row['name'] for row in ROWS if '.' in row['name']]
if len({row['name'] for row in ROWS}) != len(ROWS): raise ValueError('duplicate SDK verb')
...
"""
    for row in ROWS:
        ...
        if row.get('context') == 'board':
            result += f'return execute_mcp_board_verb(server, actor, |context| oneiron::task_verb::sdk::{row["name"].replace(".", "_")}(context, input)).await; }},\n'
            continue
        result += f'let output = oneiron::task_verb::sdk::{row["name"].replace(".", "_")}(&memory, input).map_err(mcp_facade_error)?;\n'
        if 'mcp_result' in row:
            result += f'let (value, source) = {row["mcp_result"]}; return Ok((value, source, McpCarrierPolicy::Drain, None)); }},\n'
            continue
        if 'page' in row:
            page = row['page']
...
def outputs():
    ...
    core += 'pub const SECTION_VERBS: &[&str] = &'+json.dumps([r['name'] for r in ROWS if r['section']])+';\n'
    ...
    for r in ROWS:
        ...
        if r.get('context') == 'board':
            core += f"pub fn {method}<S: crate::board_verb::LiveBoardSource>(context: &mut crate::board_verb::BoardVerbContext<'_, S>, {binding}: {r['input']}) -> Result<{r['output']}, crate::board_verb::BoardVerbError> {{ Ok({r['call']}) }}\n"
        else:
    ...
    facade=[r['name'] for r in facade_rows]
    ...
    remote += '''pub fn agent_verb(&self, verb: &str, input: serde_json::Value) -> Result<serde_json::Value, MemoryError> {
        ...
        if !FACADE_VERB_CATALOG.contains(&verb) { return Err(crate::error::bad_request("unknown SDK agent verb", &["Use a verb from the SDK catalog."])); }
...
'''
    for family in dict.fromkeys(r['name'].split('.')[0] for r in facade_rows if '.' in r['name']):
        ...
        ts += '},\n'
    ts += '}\n}\n'
    ts_native = '\n'.join(TS_NATIVE_BOUNDARIES[r['name']] if r['name'] in TS_NATIVE_BOUNDARIES else js_method(r['name'])+'(input: unknown): unknown' for r in facade_rows)
    ...
    tasks = sorted(r['name'] for r in ROWS if r['name'].startswith('tasks.'))
    ...
    board = sorted(r['name'] for r in ROWS if r.get('context') == 'board')
    board_catalog = f'pub const BOARD_VERBS: [&str; {len(board)}] = '+json.dumps(board)+';\n'
    board_catalog += '#[derive(Debug, Clone, Copy, PartialEq, Eq)]\npub enum BoardVerb { '+','.join(n.split('.')[1].title() for n in board)+' }\n'
    board_catalog += f'impl BoardVerb {{ pub const ALL: [Self; {len(board)}] = ['+','.join('Self::'+n.split('.')[1].title() for n in board)+'];\n'
    board_catalog += "pub const fn as_str(self) -> &'static str { match self {\n"
    board_catalog += ''.join('Self::'+n.split('.')[1].title()+' => '+json.dumps(n)+',\n' for n in board)+'}}}\n'
    room_catalog=HEADER+f'pub const ROOMS_VERBS: [&str; {len(rooms)}] = '+json.dumps(rooms)+';\n'
    mcp_names = [r['name'] for r in ROWS if r['mcp'] == 'tool-first']
    ...
    for function, field in [('agent_argument_fields', 'mcp_fields'), ('agent_required_fields', 'mcp_required_fields')]:
        mcp += f"fn {function}(name: &str) -> Option<&'static [&'static str]> {{ match name {{\n"
        for r in ROWS:
            if r['mcp'] == 'none': continue
            mcp += f'{json.dumps(r["name"])} => Some(&'+json.dumps(list(r.get(field, r['mcp_fields'])))+'),\n'
        mcp += '_ => None, }}\n'
    for function, select in [
        ('agent_continuable', lambda r: r.get('mcp_page', 'page' in r or r['output'].startswith('Vec<'))),
        ('agent_filtered_read', lambda r: r['mcp_scope'] == 'filtered'),
        ('agent_unscoped', lambda r: r['mcp_scope'] == 'unrestricted'),
        ('agent_writes', lambda r: r['scope'] == 'Write'),
    ]:
        names = [json.dumps(r['name']) for r in ROWS if r['mcp'] == 'tool-first' and select(r)]
        mcp += f'fn {function}(name: &str) -> bool {{ matches!(name, '+(' | '.join(names) if names else '_ if false')+') }\n'
    ...
    for path, start, end, body in [
        ('crates/oneiron/src/board_verb.rs', '// BEGIN GENERATED BOARD CATALOG', '// END GENERATED BOARD CATALOG', board_catalog),
